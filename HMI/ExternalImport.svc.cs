...
namespace HMI
{
    ...
    {
#pragma warning disable CS1591 // 遺漏公用可見類型或成員 'ExternalImport.~ExternalImport()' 的 XML 註解
        ~ExternalImport()
#pragma warning restore CS1591 // 遺漏公用可見類型或成員 'ExternalImport.~ExternalImport()' 的 XML 註解
        {
        ...
        /// <inheritdoc/>
        public void CreateDirectory([WsdlParamOrReturnDocumentation("要建立的資料夾")] string path)
        {
            bool result = true ; //建置結果
            if (!Directory.Exists(path))
            {
                Directory.CreateDirectory(path);
                result = true;
            }
            WriteInfo(WriteMemorLog, $@"建置 {path} 結果 {result}"); //寫入建立資料夾結果
            _Callback.CreateDirectoryResult(result);
        }
