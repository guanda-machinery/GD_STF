...
namespace GD_STD
{
    ...
    public static class PCSharedMemory
    {
        ...
        public static void SetValue<T>(long position, byte[] value) where T : IPCSharedMemoryOffset, new()
        {
            T result = new T();
        ...
        /// <returns></returns>
        public static IntPtr GetReadIntPtr<T>() where T : struct, IPCMemoryPointer
        {
            T result = new T();
            return result.ReadIntPtr();
        }
    }
}