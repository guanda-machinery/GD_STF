...
namespace GD_STD.Data
{
    ...
    public unsafe class Chromosome : IMatchSetting
    {
            ...
            set
            {
                ...
                if (value)
                {
                    ...
                    for (int i = 0; i < PartNumber.Length; i++)
                    {
                        ...
                        {
                            if (_Ptr[DNA[i]].Number[c] != '\0') //過濾掉不需要的字元
                            {
                    ...
                    //{
                    //    List<char> number = new List<char>();
                    //    for (int c = 0; c < 40; c++)//SinglePart Number 長度是 40 所以只能開 40
                    //    {
                    //        Debug.Write(_Ptr[i].Number[c]);
        ...
        /// <param name="mutation">配料次要長度</param>
        ///  <param name="score">評分</param>
        public Chromosome(double length, IMatchSetting setting, int index, SinglePart* parts, int count,bool mutation, IConfiguration score)
        {
        ...
        /// 可能符合的料件
        /// </summary>
        /// <param name="fitness">適合程度</param>
        /// <returns>回傳全部符合料件</returns>
        public List<SinglePart> Match(double fitness)
        {
            double max = Length - UseLength(); //最大可配置長度
            List<SinglePart> result = new List<SinglePart>();//可配置的列表
            List<double> addLength = new List<double>();//有加入過的物件長度
            for (int i = 0; i < _Count; i++) //逐步看動態指標
            {
                if (_Ptr[i].Length < max + Cut && !DNA.Contains(i) && !addLength.Contains(_Ptr[i].Length) && !_Ptr[i].Match) //找尋可配置的長度
                {
                    Chromosome chr = this.Clone();
                    chr.Add(_Ptr[i].Index);
                    if (fitness < chr.Fitness())
                    {
                        result.Add(_Ptr[i]);//加入物件
                        addLength.Add(_Ptr[i].Length);
                    }
        ...
        public SinglePart? SingleMatch()
        {
            ...
            for (int i = 0; i < _Count; i++)//逐步看動態指標
            {
                if (_Ptr[i].Length < max && !_Ptr[i].Match && !DNA.Contains(i)) //找尋可配置的長度
                {
        ...
        /// 取得所有配置過的物件長度
        /// </summary>
        /// <returns>DNA 物件長度</returns>
        public List<double> DNALength()
        {
            List<double> result = new List<double>();
            for (int i = 0; i < DNA.Count; i++)
            {
                result.Add(_Ptr[DNA[i]].Length);
            }
            return result;
        }
        /// <summary>
        ...
        public Chromosome Clone()
        {
            using (Stream objectStream = new MemoryStream())
            {
                ...
                return formatter.Deserialize(objectStream) as Chromosome;
            }
        }
        #endregion
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        internal static int Compare(SinglePart a, SinglePart b)
        {
