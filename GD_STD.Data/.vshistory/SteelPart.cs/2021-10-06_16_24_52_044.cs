...
namespace GD_STD.Data
{
    ...
    public class SteelPart : ITeklaObject, IProfile, ISecondary, IModelObjectBase, IMerge, IDrawing, ITekla, IChange, ISteelProfile, ITypeSettingPartView
    {
        ...
        /// <summary>
        /// 已構件 ID 找出 子物件的 ID 所以位置
        /// </summary>
        /// <param name="father">構件 ID 集合</param>
        /// <returns></returns>
        public List<int> PartIndex(List<int> father)
        {
            List<int> result = new List<int>();
            //foreach (var id in father) //逐步展開購件 id
            //{
            ...
            //    }
            //}
            Parallel.ForEach(father , el=> 
            {
                int index = this.ID.IndexOf(el);
                if (index != -1) //找到構件的零件
                {
                    result.Add(index);
                }
