...
namespace GD_STD.Data
{
    ...
    /// </summary>
    [Serializable]
    public class SteelPart : ITeklaObject, IProfile, IPart, ISecondary, IModelObjectBase, IMerge, IDrawing ,ITekla
    {
        public SteelPart()
        {
            ID = new List<int>();
        ...
        public void Merge(object obj)
        {
            if (obj is SteelPart steel)
            {
                ...
                if (GUID != null)
                {
                    State = DRAWING_STATE.INCREASE_COUNT;
                    Revise = DateTime.Now;
                }
                Count++;
            }
        ...
        {
            int index = Father.Find(el => el == id);
            Father.Remove(index);
            Count--; 
        }
