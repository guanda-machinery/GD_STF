...
namespace GD_STD.Data
{
    ...
    public unsafe class Population : List<Chromosome>
    {
        ...
        /// <param name="parts">物件列表</param>
        /// <param name="count">陣列數量</param>
        ///  <param name="score">評分</param>
        public Population(MatchSetting match, SinglePart[] parts, int count, IScore score)
        {
            _Match = match;
        ...
        public void Run()
        {
            while (_Parts.Where(el => !el.Match).Count())
            {
        ...
        private void Initialize()
        {
            ...
            {
                Seed.Add(new Chromosome(lengths[i], _Match, index, _Parts, _Count, _Score));//將主要配料長度加入到種子列表內
            }
        }
        #endregion
