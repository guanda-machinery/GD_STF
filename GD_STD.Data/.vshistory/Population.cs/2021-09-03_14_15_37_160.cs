...
namespace GD_STD.Data
{
    ...
    public unsafe class Population : List<Chromosome>
    {
        ...
        private void Suitable(bool cycle = false)
        {
            ...
            Seed.Sort(MaxCompare);//排序適合程度大到小
            if (Seed[0].Fitness() == 0 && !cycle) //如果素材短於配料長度
            {
                Initialize(_Match.SecondaryLengths);
                Suitable(true);
                return;
            }
            Seed[0].Number = _Match.StartNumber + (this.Count + 1).ToString().PadLeft(4, '0');
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        internal static int MaxCompare(Chromosome a, Chromosome b)
        {
