...
namespace GD_STD.Data
{
    ...
    public unsafe class Population : List<Chromosome>
    {
        ...
        public void Run()
        {
            if (_Count == 0)//如果沒數量
                return;//終止運算
            while (Initialize()) // Initialize = true 才會執行
            {
        ...
        public IEnumerable<IGrouping<double, Chromosome>> GroupLength()
        {
          var _group =
            ...
            var _ = from el in _group
                    let le = (from el2 in this where el.Key == el2.Length select el2).ToList()
                    select le;
            foreach (var result in _group) //疊代器
            {
                yield return result;
            }
        }
