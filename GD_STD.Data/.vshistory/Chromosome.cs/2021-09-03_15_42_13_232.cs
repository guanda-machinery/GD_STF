...
namespace GD_STD.Data
{
    ...
    public unsafe class Chromosome : IMatchSetting
    {
        ...
        public List<SinglePart> Match(double fitness, List<Chromosome> convergence)
        {
            ...
            {
                if (_Ptr[i].Length < max + Cut && !DNA.Contains(i) && !addLength.Contains(_Ptr[i].Length)) //找尋可配置的長度
                {
                    Chromosome chr = this.Clone();
                    chr.Add(_Ptr[i].Index);
                    bool add = true;
                    for (int c = 0; c < convergence.Count; c++)
                    {
                        if (convergence[c].DNALength().SequenceEqual(chr.DNALength()) && chr.Length == convergence[c].Length)
                        {
