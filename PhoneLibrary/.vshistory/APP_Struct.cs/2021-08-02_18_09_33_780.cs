...
namespace GD_STD.Phone
{
    ...
    ///             <item><see cref="TraverseGroup"/></item>
    ///         </list>
    ///     </revision>
    /// </revisionHistory>
    /// <example>
    ...
    public struct APP_Struct : IPhoneSharedMemory, IPCSharedMemory
    {
        ...
        [MarshalAs(UnmanagedType.I1)]
        [PhoneCondition(false, nameof(Origin))]
        [PhoneCondition(SELECT_DEVICE.NULL, nameof(SelectMove), false)]
        ...
        /// <exception cref="MemoryException">寫入失敗。因 Operating.Satus 狀態不是 MANUAL。</exception>
        /// <exception cref="MemoryException">寫入失敗。因欄位 Origin 的值，不符合 false 條件</exception> 
        /// <exception cref="MemoryException">寫入失敗。因欄位 Count 的值，不能小於 2</exception> 
        [DataMember]
        [PhoneCondition(false, nameof(Origin))]
        [PhoneCondition(SELECT_DEVICE.NULL, nameof(SelectMove), false)]
        public short Count;
        ...
        /// <exception cref="MemoryException">寫入失敗。因 Operating.Satus 狀態不是 MANUAL。</exception>
        /// <exception cref="MemoryException">寫入失敗。因欄位 Origin 的值，不符合 false 條件</exception> 
        /// <exception cref="MemoryException">寫入失敗。因欄位 SelectMove 的值不能是 <see cref="SELECT_DEVICE.NULL"/></exception> 
        [DataMember]
        [PhoneCondition(SELECT_DEVICE.NULL, nameof(SelectMove), false)]
        ...
        void IPhoneSharedMemory.WriteMemory()
        {
            ...
            PhoneConditionFactory<APP_Struct> phoneConditionFactory = new PhoneConditionFactory<APP_Struct>();
            if (phoneConditionFactory.Condition(this, initial))
            {
                WriteMomory();
            }
        }
