...
#endif
    {
        ...
        unsafe void IPCSharedMemory.WriteMemory()
        {
            ...
            using (var memory = MonitorWorkMemory.CreateViewAccessor(0, size, MemoryMappedFileAccess.Write))
            {
                ...
                //memory.WriteArray(0, result, 0, size);
                IntPtr intPtr = Marshal.AllocHGlobal(size); //使用指定的位元組數目，從處理序的 Unmanaged 記憶體中配置記憶體。
                Marshal.StructureToPtr(this, intPtr, true);//從指定類型的 Managed 物件封送處理資料到 Unmanaged 記憶體區塊。
                byte* writePtr = (byte*)intPtr;
