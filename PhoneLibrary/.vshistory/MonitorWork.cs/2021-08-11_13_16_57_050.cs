...
namespace GD_STD.Phone
{
...
#endif
    {
        ...
        void IPhoneSharedMemoryOffset.WriteMemory<T>(long position, T value)
        {
            ...
            #region 檢測寫入
            bool isWrite = false;//寫入
            Type type = typeof(WorkMaterial);
            long materialNumberOffsetStart = Marshal.OffsetOf(type, nameof(GD_STD.Phone.WorkMaterial.MaterialNumber)).ToInt32(); //素材編號起始位置
            long materialNumberOffsetCount = new WorkMaterial().GetSizeof(nameof(GD_STD.Phone.WorkMaterial.MaterialNumber));//素材編號 Szieof
            long smeltingNumberOffsetStart = Marshal.OffsetOf(type, nameof(GD_STD.Phone.WorkMaterial.SmeltingNumber)).ToInt32(); //熔煉號 (爐號) 起始位置
            long smeltingNumberOffsetCount = new WorkMaterial().GetSizeof(nameof(GD_STD.Phone.WorkMaterial.SmeltingNumber));//熔煉號 (爐號) Szieof
            long idOffsetStart = Marshal.OffsetOf(type, nameof(GD_STD.Phone.WorkMaterial.ID)).ToInt32(); //素材 ID 起始位置
            long idOffsetEnd = new WorkMaterial().GetSizeof(nameof(GD_STD.Phone.WorkMaterial.ID)); //素材 ID 結束位置
            long arrayLength = this.ArrayLength(nameof(this.WorkMaterial)); //陣列長度
            long writeStart = position; //寫入起始點
            long writeCount = Marshal.SizeOf(typeof(T));//寫入結束點

            for (int i = 0; i < arrayLength; i++)
            {
                if (materialNumberOffsetStart >= writeStart && writeCount <= materialNumberOffsetCount ||
                    smeltingNumberOffsetStart >= writeStart && writeCount <= smeltingNumberOffsetCount ||
                    idOffsetStart >= writeStart && writeCount <= idOffsetEnd)
                {
                    isWrite = true;
                }
            }
