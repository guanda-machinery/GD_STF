...
namespace GD_STD.Phone
{
...
#endif
    {
        ...
        public static MonitorWork Initialization()
        {
            ...
            //初始化 = -1
            short[] index = new short[result.ArrayLength(nameof(result.Index))].
        ...
        void IPhoneSharedMemoryOffset.WriteMemory(long position, byte[] value)
        {
            ...
            long indexLength = new MonitorWork().ArrayLength(nameof(MonitorWork.Index));
            long indexStart = Marshal.OffsetOf(typeof(MonitorWork), nameof(MonitorWork.Current)).ToInt64(); // index 起始位置
            ...
            long indexOffsetStart = indexStart + Marshal.SizeOf(indexType) * current;
            long indexCount = Marshal.SizeOf(indexType) * (indexLength - current);
#if !LogicYeh
            Ban = false;
#endif
            #endregion
