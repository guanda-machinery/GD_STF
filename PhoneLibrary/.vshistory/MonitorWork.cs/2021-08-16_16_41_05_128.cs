...
namespace GD_STD.Phone
{
...
#endif
    {
        ...
        void IPhoneSharedMemoryOffset.WriteMemory(long position, byte[] value)
        {
            ...
            Type indexType = Initialization().Index.GetType().GetElementType(); // Index 陣列內容 TYPE
            ...
            if ((indexOffsetStart == position && value.Length <= indexCount))
            {
                ...
                for (int i = 0; i < value.Length; i += indexElelmentSzize)
                {
                    ...
                    short result = Convert.ToInt16(vs);
...
#if LogicYeh
        unsafe void IPCSharedMemory.WriteMemory()
        {
            int size = Marshal.SizeOf(typeof(MonitorWork));
            using (var memory = MonitorWorkMemory.CreateViewAccessor(0, size, MemoryMappedFileAccess.Write))
            {
                byte[] value = this.ToByteArray();
                memory.WriteArray<byte>(0, value, 0, size);

                //byte* writePtr = (byte*)Marshal.AllocHGlobal(size);
                //byte* memoryPtr = (byte*)memory.SafeMemoryMappedViewHandle.DangerousGetHandle().ToPointer();
                //for (int i = 0; i < size; i++)
                //{
                //    memoryPtr[i] = writePtr[i];
                //}
                
                //Marshal.FreeHGlobal((IntPtr)writePtr);
            }
        }
