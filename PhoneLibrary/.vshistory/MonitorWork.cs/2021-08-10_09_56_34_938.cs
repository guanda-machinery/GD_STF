...
#endif
    {
        ...
        T ISharedMemoryOffset.ReadMemory<T>(long position, int size)
        {
            using (var memory = MonitorWorkMemory.CreateViewAccessor(0, size, MemoryMappedFileAccess.Read))
            {
                ...
                memory.ReadArray<byte>(position, buffer, 0, (int)position + size);
                return (T)Marshal.PtrToStructure(memory.SafeMemoryMappedViewHandle.DangerousGetHandle(), typeof(T));
            }
        }
        /// <summary>
        /// 讀取記憶體
