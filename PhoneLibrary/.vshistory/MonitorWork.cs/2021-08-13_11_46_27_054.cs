...
#if LogicYeh
        , IPCSharedMemory, IPCSharedMemoryOffset, IMemoryPointer
#endif
    {
        /// <summary>
        /// 初始化結構
        ...
        void IPhoneSharedMemoryOffset.WriteMemory(long position, byte[] value)
        {
            ...
            Type indexType = Initialization().Index.GetType().GetElementType(); // Index 陣列 TYPE
            ...
            long indexOffsetStart = indexStart + Marshal.SizeOf(indexType) * (current + 1);
            long indexCount = Marshal.SizeOf(indexType) * (indexLength - (current + 1));
#if !LogicYeh
            Ban = false;
#endif
            #endregion
            ...
            if ((writeMaterialNumber == 0 && value.Length <= materialNumberOffsetCount) ||
                ...
                (indexOffsetStart == position && value.Length <= indexCount))
            {
        ...
        /// 
        /// </summary>
        /// <param name="offset">讀取位置</param>
        /// <param name="readLength">讀取長度</param>
        /// <param name="fieldName">欄位名稱</param>
        /// <returns></returns>
        public static char[] ReadChar(long offset, int readLength, string fieldName)
        {
            int size = Marshal.SizeOf(typeof(MonitorWork).GetType());
            using (MemoryMappedViewAccessor view = MonitorWorkMemory.CreateViewAccessor(0, size, MemoryMappedFileAccess.Read))
            {
                IntPtr intPtr = view.SafeMemoryMappedViewHandle.DangerousGetHandle(); //記憶體指標
                //int offset = Marshal.OffsetOf(typeof(MonitorWork), nameof(MonitorWork.ProjectName)).ToInt32(); //偏移位置
                //int nameLength = typeof(MonitorWork).ArrayLength(nameof(MonitorWork.ProjectName)); //ProjectName 長度
                FieldInfo info = typeof(MonitorWork).GetField(fieldName);
                if (true)
                {

                }
                Type elementType = typeof(MonitorWork).GetField(fieldName).FieldType.GetElementType(); //陣列節點 type
