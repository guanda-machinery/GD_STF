...
namespace GD_STD.Phone
{
    ...
    public struct MonitorWork : ISharedMemory, Base.ISharedMemory
    {
        ...
        public MonitorWork(char[] projectName = null, int count = 0, ushort schedule = 0, int[] index = null, WorkMaterial[] workMaterial = null, WorkMaterial[] matchWorkMaterial = null)
        {
            ...
            if (index != null)
                if (index.Length == result.Index.Length)
                {
                    ...
                    if (index.Select(el => el == 0).Count() >= 2)
                    {
                        index.GroupBy(el => el).ToList();
