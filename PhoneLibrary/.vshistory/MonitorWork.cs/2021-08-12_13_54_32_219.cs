...
namespace GD_STD.Phone
{
...
#endif
    {
        ...
        public static MonitorWork Initialization()
        {
            ...
            WorkMaterial[] work = new WorkMaterial[typeof(WorkMaterial).ArrayLength(nameof(result.WorkMaterial))];
            //初始化 = -1
            short[] index = new short[typeof(WorkMaterial).ArrayLength(nameof(result.Index))].
                                            Select(el => el = -1).ToArray();
            result.Index = index;
            result.ProjectName = new ushort[typeof(WorkMaterial).ArrayLength(nameof(result.ProjectName))];
#if !LogicYeh
            Ban = false;
#endif
            #endregion
        ...
        IntPtr IPCMemoryPointer.ReadIntPtr()
        {
            int size = Marshal.SizeOf(this.GetType());
            using (var memory = MonitorWorkMemory.CreateViewAccessor(0, size, MemoryMappedFileAccess.Read))
                return memory.SafeMemoryMappedViewHandle.DangerousGetHandle();
        }
#endif
        #endregion
        #endregion
