...
namespace GD_STD.Phone
{
    ...
    public static class SharedMemory
    {
        ...
        public static T SetValue<T, TVaule>(long position, TVaule value, int valueOffset, int count) where T : IPhoneSharedMemoryOffset, ISharedMemory, new() where TVaule : struct
        {
            T result = new T();
            result.WriteMemory<TVaule>(position, value, valueOffset, count);
            result.ReadMemory();
            return result;
        }
        /// <summary>
            ...
            foreach (var field in fieldInfos)
            {
                if (field.FieldType.IsArray)
                {
                    ...
                    if (split[0] == $"{field.Name}[{arrayIndex}]")
                    {
                        if (nextField != "")
                        {
                            object nextObj = Activator.CreateInstance(arrayType);
