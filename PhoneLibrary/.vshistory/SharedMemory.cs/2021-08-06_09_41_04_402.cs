        ...
        public static long GetMemoryOffset<T>(T obj, string fieldName) where T : IPhoneSharedMemoryOffset
        {
            ...
            foreach (var field in fieldInfos)
            {
                if (field.FieldType.IsArray)
                {
                    ...
                    arrayLength = field.FieldType.ArrayLength(field.Name);
                    string strCurrent = split[0].Substring(split[0].IndexOf('['), split[0].IndexOf('['));
                    arrayIndex = int.TryParse(strCurrent);
                }
            ...
            //        }
            //        result += GetMemoryOffset<T>(obj, fieldName);
            //        return result;
            //    }
            //}
