...
namespace GD_STD.Phone
{
    ...
    public static class SharedMemory
    {
        ...
        public static long GetMemoryOffset<T>(T obj, string fieldName) where T : IPhoneSharedMemoryOffset
        {
            ...
            string[] split;
            string nextField;
            GetFieldStr(obj, fieldName, out fieldInfos, out split, out nextField);
            ...
            foreach (var field in fieldInfos)
            {
                if (field.FieldType.IsArray)
                {
                    ...
                    if (split[0] == $"{field.Name}[{arrayIndex}]")
                    {
                        if (nextField != "")
                        {
                            return Next(arrayType, nextField);
                        }
                        return result;
                    }
                }
                ...
                {
                    Next(field.FieldType, nextField);
                    return result;
                }
                else
                {
        ...
        private static void GetFieldStr<T>(T obj, string fieldName, out FieldInfo[] fieldInfos, out string[] split, out string nextField) where T : IPhoneSharedMemoryOffset
        {
