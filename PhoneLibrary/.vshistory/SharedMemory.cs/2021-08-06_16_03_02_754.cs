...
namespace GD_STD.Phone
{
    ...
    public static class SharedMemory
    {
        ...
        public static long GetMemoryOffset<T>(T obj, string fieldName)
        {
            ...
            {
                if (field.FieldType.IsEnum)//枚舉
                {

                }
                else if (field.FieldType.IsArray)//陣列
                {
                    int arrayLength;//欄位陣列的總長
                    arrayLength = obj.ArrayLength(field.Name);
                    MatchCollection currentName = Regex.Matches(split[0], @"\b[0-9a-zA-Z]");
                    string name = split[0].Substring(0, split[0].IndexOf('['));
                    if (name == field.Name)
                    {
                        int arrayIndex;//指定欄位的陣列位置
                        MatchCollection matches = Regex.Matches(split[0], @"\b[0-9]+?");
