        ...
        public static long GetMemoryOffset<T>(T obj, string fieldName) where T : IPhoneSharedMemoryOffset
        {
            ...
            foreach (var field in fieldInfos)
            {
                if (field.FieldType.IsArray)
                {
                    ...
                    MatchCollection matches = Regex.Matches(split[0], @"[\d]?");
                    //string strCurrent = split[0].Substring(split[0].IndexOf('[')+1, split[0].Length - split[0].IndexOf(']'));

                    int.TryParse(matches[0].Value, out arrayIndex);
                }
