...
namespace GD_STD.Phone
{
    ...
    public static class SharedMemory
    {
        ...
        public static long GetMemoryOffset<T>(T obj, string fieldName) where T : IPhoneSharedMemoryOffset
        {
            ...
            string nextField = string.Empty;//下一層欄位
            //如果有下一層
            if (split.Length >= 2)
            {
            ...
            foreach (var field in fieldInfos)
            {
                if (field.FieldType.IsArray)
                {
                    ...
                    arrayLength = newObj.ArrayLength(field.Name);
                    ...
                    if (fieldName == field.Name)
                    {
                        ...
                        return Next(field.FieldType, nextField, result);
                    }
                }
        ...
        public static long Next(Type type, string fieldName, long size)
        {
            object obj = Activator.CreateInstance(type);
            if (fieldName != "")
            {
                size += GetMemoryOffset((IPhoneSharedMemoryOffset)obj, fieldName);
                return size;
            }
            return size;
        }
