...
namespace GD_STD.Phone
{
    ...
    public static class SharedMemory
    {
        ...
        public static long GetMemoryOffset<T>(T obj, string fieldName) where T : IPhoneSharedMemoryOffset
        {
            ...
            int indexOf = fieldName.IndexOf('.'); //目前取得欄位名稱的的中斷點
            string node = fieldName.Substring(0, indexOf + 1);//當前欄位名稱包含陣列索引
            ...
            string strArrayIndex = node.Split('[', ']').Aggregate((str1, str2) => $"{str1}{str2}");//取得文字內的陣列索引
