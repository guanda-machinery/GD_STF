        ...
        public static long GetMemoryOffset<T>(T obj, string fieldName) where T : IPhoneSharedMemoryOffset
        {
            ...
            foreach (var field in fieldInfos)
            {
                if (field.FieldType.IsArray)
                {
                    ...
                    result += Marshal.SizeOf(field.ReflectedType) * arrayIndex;
                    if (fieldName == field.Name.Insert(field.Name.Length -2, arrayIndex.ToString()))
                    {
                        return Next(field.FieldType, nextField);
                    }
                }
