...
namespace GD_STD.Phone
{
    internal class PhoneConditionFactory<T> where T : struct, IPhoneSharedMemory
    {
        ...
        /// <returns></returns>
        private object SelectedConditionsValue(string referFieldName, FieldInfo fieldInfo)
        {
            return el.IsConditionInitial ? _Compare.GetType().GetField(referFieldName).GetValue(_Initial) : _Compare.GetType().GetField(referFieldName).GetValue(_Compare);
        }
