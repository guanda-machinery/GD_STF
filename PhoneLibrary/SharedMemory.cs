...
namespace GD_STD.Phone
{
    ...
    public static class SharedMemory
    {
        ...
        /// <returns></returns>
        private static MemoryMappedViewAccessor GetReadView<T>() where T : struct, IMemoryPointer
        {
#if !LogicYeh
            if (!Ban)
        ...
        public static char[] ReadChar<T>(long offset, int readLength, string fieldName) where T : struct, IMemoryPointer
        {
            ...
            unsafe
            {
                byte* ptr = (byte*)intPtr.ToPointer(); //byte*
                for (int i = 0; i < readLength; i++)  //逐步執行讀取記憶體內的 byte
                {
                    fixed (void* p = &buffer[i]) //  p 指標指向 buffer[i]
                    {
                        GetVoid(p, ptr, offset + i * elementSize, elementSize);
                    }
                }
            }
            _View.Dispose();
        ...
        /// <summary>
        /// 讀取 <typeparamref name="T"/> 記憶體內值定的位置，並使用 unsafe 轉換 <see cref="short"/>[] 
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="offset"></param>
        /// <param name="readLength"></param>
        /// <param name="fieldName"></param>
        /// <returns></returns>
        public static short[] ReadShort<T>(long offset, int readLength, string fieldName) where T : struct, IMemoryPointer
        {
#if !LogicYeh
            if (!Ban)
                throw new Exception("使用失敗,因不是測試專案。");
            ...
            Type elementType = typeof(T).GetField(fieldName).FieldType.GetElementType(); //陣列節點 type
            int elementSize = Marshal.SizeOf(elementType); //陣列節點 sizeof
            short[] buffer = new short[readLength]; //讀取資料長度
            unsafe
            {
                byte* ptr = (byte*)intPtr.ToPointer(); //byte*
                for (int i = 0; i < readLength; i++)  //逐步執行讀取記憶體內的 byte
        ...
        /// 取得無型別指標
        /// </summary>
        /// <param name="p">無型別指標</param>
        /// <param name="ptr">記憶體指標</param>
        ...
        public static unsafe void GetVoid(void* p, byte* ptr, long offset, int elementSize)
        {
            ...
            for (int c = 0; c < elementSize; c++) //塞入到 buffer[i][offset + c]
            {
                p1[c] = ptr[offset + c];
            }
        }
