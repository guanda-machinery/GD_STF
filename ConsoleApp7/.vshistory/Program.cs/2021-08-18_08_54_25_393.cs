...
namespace ConsoleApp7
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            intPtr = CreateFile("\\\\.\\AdvLmDev", FileAccess.Write, FileShare.None, IntPtr.Zero, FileMode.Open, FileOptions.None, IntPtr.Zero);
            if (intPtr == (IntPtr)(-1))
            {
                Console.Write("錯誤");
                Console.ReadLine();
            ...
            while (true)
            {
                ...
                bool ok = DeviceIoControl(intPtr, IOCTL_LMSENSOR_GET_VOLT, 0, 0, ref voltage, Marshal.SizeOf(typeof(ulong)), IntPtr.Zero, 0);
                if (!ok)
                {
                    Console.WriteLine("撈不到");
                    Console.ReadLine();
                    return;
                }
                for (int i = 0; i < voltage.Length; i++)
                {
                    Console.WriteLine(voltage[i]);
                }
            }
        }
    }
