...
namespace WPFSTD105.Tekla
{
    ...
    public class TeklaHtemlFactory
    {
        ...
        {
            _BomPath = bomPath;
            Material = SerializationHelper.GZipDeserialize<ObservableCollection<SteelMaterial>>(ApplicationVM.ModelFileMaterial()); //系統內材質序列化檔案
                                ...
                                if (KeyValuePairs.ContainsKey(key))//如果找到相同的 key
                                {
                                    ...
                                    if (obj.GetType() == typeof(SteelPart)) //如果類型是 SteelPart
                                    {
                                            ...
                                            part.Type == OBJETC_TYPE.L)
                                        {
                                            ...
                                            if (Material.Where(el => el.Name == part.Material).Count() > 0) //如果沒有模型材質列表沒有相對應的材質
                                            {
