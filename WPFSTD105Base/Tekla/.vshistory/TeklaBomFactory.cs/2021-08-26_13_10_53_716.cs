            ...
            for (int i = 1; i < data.Length; i++)
            {
                ...
                try
                {
                    ...
                    else if (info.PropertyType.IsGenericType)
                    {
                        ...
                        {
                            Type genericType = info.PropertyType.GetGenericTypeDefinition();
                            ...
                            MethodInfo _ = info.PropertyType.GetMethod("Add");
                            _.Invoke(info.PropertyType, new object[] { Convert.ChangeType(strValue, typeArgs[0]) });
                            //Type _ = info.PropertyType.GetGenericTypeDefinition();
                            //Type[] typeArgs = info.PropertyType.GetGenericArguments();//取得封閉泛型類型的型別引數
                            //Type constructed = _.MakeGenericType(typeArgs); //用類型陣列的項目取代目前泛型類型定義的型別參數，並傳回代表所得結果建構類型的 System.Type 物件。
                            //IList list = (IList)Activator.CreateInstance(constructed);//反射出集合
                            //list.Add(Convert.ChangeType(strValue, typeArgs[0]));//加入到集合內
                            //setValue = list;
                        }
                        else
                        {
