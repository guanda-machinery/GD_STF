                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (number > 2)
                    {
                        ...
                        else if (objType == Father.GetType())//如果是構件
                        {
                            ...
                            {
                                if (steels.ElementAt(0).Equals(Father)) //檢查雜湊
                                {
                                    
                                }
                                else
                                {
                                    MessageBox.Show($"雜湊不相符，在資料行 {number} 。\n請檢查報表是否有相同構件編號，但長度寬度不相符。", "錯誤", MessageBoxButton.OK, MessageBoxImage.Error, MessageBoxResult.None, MessageBoxOptions.ServiceNotification);
                                }
                            }
                            else //如果找不到相同物件
                            {
                                _SteelAssemblies.Add(Father);
                            }
                        }
                        else
                        {
