...
namespace WPFSTD105.Tekla
{
    ...
    public class TeklaHtemlFactory
    {
        ...
        public TeklaHtemlFactory(string path)
        {
            ...
            if (logo == null)
            {
                throw new Exception("無法讀取檔案 : 此表格有可能是非來自 Tekla Structures 所輸出，或者報表作者不是來自 GUANDA");
            }
            else
            {
                LoadProjectdInfo();
        ...
        #region 私有方法
        private void LoadProjectdInfo()
        {
            _doc.DocumentNode.SelectNodes("//table").
                Where(table => table.GetAttributeValue("name", "") == "ProjectInfo"). //篩選出 <table Name="ProjectInfo" 位置(複製貼上可找到位置)
                    ToList().
                        ForEach(tableContent =>
                        {
                            tableContent.SelectNodes("tfoot/tr/td"). //找到指定節點
                                Where(el => el.GetAttributeValue("name", "") != ""). //過濾沒有 name 參數設定
                                    ForEach(el =>
                                    {
                                        string strValue = Regex.Replace(el.InnerText, @"<[^>]+>|&nbsp;", "").Trim();//將 html 格式的空白字元拿掉
                                        PropertyInfo propertyInfo = this.GetType().GetProperty(el.GetAttributeValue("name", ""));
                                        object setValue = Convert.ChangeType(strValue, propertyInfo.PropertyType);//反射出屬性 TYPE ，並轉換要賦予的值
                                        propertyInfo.SetValue(this, setValue);
                                    });
                                ...
                                if (result != null)
                                {
                                    ...
                                    if (value != "")
                                    {
                                        ...
                                        if (propertyInfos.Count() != 0)
                                        {
                                            ...
                                            {
                                                //判斷此屬性是否有需要過濾
                                                if (filter != null && filter.ContainsKey(propertyInfo.PropertyType))
                                                {
        ...
        public List<object> Load()
        {
            List<object> result = new List<object>();
            //取得表格節點
            _doc.DocumentNode.SelectNodes("//table").ToList().//只選擇指定資料表
                   ForEach(tableContent =>
                   {
                       HtmlNodeCollection htmlNodes = null;
                       var tbodyNodes = tableContent.SelectNodes("tbody");//共用欄位參數
                       ...
                       //      object val = Cell(tfoot.ChildNodes, typeof(SteelAttr), null);
                       //      if (val != null)
                       //      {
                       //          result.Add(val);
                       //      }
