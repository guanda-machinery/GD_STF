            ...
            _doc.DocumentNode.SelectNodes("//table").
                Where(table => table.GetAttributeValue("name", "") == "ProjectInfo"). //篩選出 <table Name="ProjectInfo" 位置(複製貼上可找到位置)
                    ToList().
                        ...
                        {
                            tableContent.ChildNodes.ToList().Where(el => el.Name == "td").
                                Where(td => td.GetAttributeValue("name", "") != "").
                                    ForEach(td =>
                                    {
                                        PropertyInfo info = this.GetType().GetProperty(td.GetAttributeValue("name", ""));//取得報表對應反射欄位
                                        string strValue = Regex.Replace(td.InnerText, @"<[^>]+>|&nbsp;", "").Trim();//將 html 格式的空白字元拿掉
                                        object setValue = Convert.ChangeType(strValue, info.PropertyType);//反射出屬性 TYPE ，並轉換要賦予的值
                                        this.GetType().GetField(td.GetAttributeValue("name", "")).SetValue(this, setValue);
                                    });
                        });
        }
        #endregion
