...
namespace WPFSTD105.Tekla
{
    ...
    public class TeklaHtemlFactory
    {
        ...
        /// 軟體缺少的斷面規格列表
        /// </summary>
        public Dictionary<OBJETC_TYPE, ObservableCollection<SteelAttr>> Profile = new Dictionary<OBJETC_TYPE, ObservableCollection<SteelAttr>>();
        /// <summary>
        /// 軟體缺少的材質
        /// </summary>
        ...
        /// 有增加斷面規格
        /// </summary>
        private bool _AddProfile { get; set; }
        /// <summary>
        /// 有增加過材質
        /// </summary>
        private bool _AddMaterial { get; set; }
        #endregion
        /// <summary>
        /// 處理 Tekla CSV 表格
        /// </summary>
        ...
        {
            _BomPath = bomPath;
            Material = SerializationHelper.GZipDeserialize<ObservableCollection<string>>(ApplicationVM.ModelFileMaterial()); //系統內材質序列化檔案
                            ...
                            else
                            {
                                ...
                                if (KeyValuePairs.ContainsKey(key))//如果找到相同的 key
                                {
                                    ...
                                    if (obj.GetType() == typeof(SteelPart)) //如果類型是 SteelPart
                                    {
                                        ...
                                        {
                                            if (Profile[part.Type].Where(el => el.Profile == part.Profile).Count() == 0)//如果模型找不到相同的斷面規格
                                            {
                                                /*自動新增斷面規格*/
                                                SteelAttr att = new SteelAttr((SteelPart)part); //轉換斷面規格設定檔
                                                Profile[part.Type].Add(att);//加入到列表內
                                            }
                                            if (Material.IndexOf(part.Material) == -1) //如果沒有模型材質列表沒有相對應的材質
                                            {
                                                Material.Add(part.Material);//加入到材質
                                            }
                                        }
                                    }
