...
namespace WPFSTD105.Tekla
{
    ...
    public class TeklaHtemlFactory
    {
        ...
        /// 構件資訊列表
        /// </summary>
        private ObservableCollection<SteelAssembly> _SteelAssemblies = new ObservableCollection<SteelAssembly>();
        /// <summary>
        /// 次件列表
        /// </summary>
        private Dictionary<string, ObservableCollection<object>> _KeyValuePairs = new Dictionary<string, ObservableCollection<object>>();
            ...
            using (StreamReader reader = new StreamReader(bomPath, System.Text.Encoding.Default))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (number > 2)
                    {
                        ...
                        {
                            MessageBox.Show($"轉換失敗因 {strTtype} 不是有效物件。", "錯誤", MessageBoxButton.OK, MessageBoxImage.Error, MessageBoxResult.None, MessageBoxOptions.ServiceNotification);
                            return;//離開函式
                        }
                        ...
                        else if (objType == Father.GetType())//如果是構件
                        {
                            Father = (SteelAssembly)obj;//存取物件報表是樹狀圖的所以需要用到此物件的部分資訊
                            var steels = _SteelAssemblies.Where(el => el.Number == Father.Number); //只篩選構件編號
                            if (steels.Count() > 0) //如果找到相同物件
                            {
                                ...
                                {
                                    goto HashCodeError; //跳脫邏輯到 HashCodeError 標籤
                                }
                            }
                            else //如果找不到相同物件
                            {
                                _SteelAssemblies.Add(Father);//加入物件
                            }
                        }
                        else
                        {
                            string key = ((IProfile)obj).Profile; //字典 key 
                            if (_KeyValuePairs.ContainsKey(key))//如果找到相同索引值
                            {
                                int index = 0;//索引位置
                                if (obj.GetType() == typeof(SteelPart))
                                {
                                    index = _KeyValuePairs[key].FindIndex(el => el.GetType() == typeof(SteelPart) && ((SteelPart)el).Number == ((SteelPart)obj).Number);//找出字典檔內物件
                                }
                                else
                                {
                                    index = _KeyValuePairs[key].FindIndex(el => el.GetType() == typeof(SteelBolts) && ((SteelBolts)el).Profile == ((SteelBolts)obj).Profile);//找出字典檔內物件
                                }
                                if (index == -1) //如果找不到物件
                                {
                                    _KeyValuePairs[key].Add(obj);//將物件加入字典
                                }
                                else
                                {
                                    if (_KeyValuePairs[key][index].GetHashCode() == obj.GetHashCode())//使用雜湊確認合併物件
                                    {
                                        ((IMerge)_KeyValuePairs[key][index]).Merge(obj);
                                    }
                                    else
                                    {
                                        goto HashCodeError;
