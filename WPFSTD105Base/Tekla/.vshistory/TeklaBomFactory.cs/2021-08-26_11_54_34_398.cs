                ...
                try
                {
                    ...
                    else if (info.PropertyType.IsGenericType)
                    {
                        if (info.PropertyType.GetInterfaces().Contains(typeof(IList)))
                        {
                            ...
                            //Type constructed = info.PropertyType.MakeGenericType(); //用類型陣列的項目取代目前泛型類型定義的型別參數，並傳回代表所得結果建構類型的 System.Type 物件。
                            IList list = (IList)Activator.CreateInstance(info.PropertyType.GetGenericTypeDefinition());//反射出集合
                            list.Add(Convert.ChangeType(strValue, info.PropertyType));//加入到集合內
                            setValue = list;
                        }
                        else
                        {
