    ...
    public class TeklaHtemlFactory
    {
        ...
        public TeklaHtemlFactory(string bomPath)
        {
            ...
            using (StreamReader reader = new StreamReader(bomPath, System.Text.Encoding.Default))
            {
                ...
                {
                    //標頭 0~2 是模型資訊
                    if (number > 2 && ToObject(line, number) == null)
                    {
                        return;
                    }
                    else
        ...
        private object ToObject(string str, int number)
        {
            ...
            {
                IEnumerable<FieldInfo> fieldInfos = type.GetFields().Where(el => el.GetCustomAttribute<TeklaBomAttribute>().Index == i);//收尋對應欄位
                try
                {
#if DEBUG
                    //欄位是唯一不然設中斷點
...
#endif
                    FieldInfo info = fieldInfos.ElementAt(0);//收尋到的對應欄位只有唯一個
                    object value = Convert.ChangeType(data[i], info.FieldType);//轉換報表內的參數
