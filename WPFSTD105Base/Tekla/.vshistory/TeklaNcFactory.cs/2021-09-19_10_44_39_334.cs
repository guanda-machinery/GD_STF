                    ...
                    using (StreamReader stream = new StreamReader(path, Encoding.Default))
                    {
                        ...
                        while ((line = stream.ReadLine()) != null)
                        {
                            ...
                            if (save) //檔案要序列化
                            {
                                ...
                                Steel3DBlock steelBlock = new Steel3DBlock(Steel3DBlock.GetProfile(steelAttr)); //產生鋼構圖塊
                                model.Blocks.Add(steelAttr);//加入鋼構圖塊到模型
                                BlockReference blockReference = new BlockReference(0, 0, 0, steelAttr.Name, 1, 1, 1, 0);//產生鋼構參考圖塊
