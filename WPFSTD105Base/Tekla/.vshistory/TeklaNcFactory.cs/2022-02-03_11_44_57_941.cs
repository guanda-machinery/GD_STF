            ...
            switch ((NcLine)line)
            {
                ...
                case NcLine.SI:
                    ...
                    if (File.Exists($@"{ApplicationVM.DirectorySteelPart()}\{dataName}.lis")) //判斷是否有存在此檔案
                    {
                        ...
                        string[] tear = str.Split(' ');
                        string number = tear[tear.Length - 1].Split('#')[0];
                        int index = steelParts.FindIndex(el => el.Number == number);//列表位置
                        if (index != -1) //報表內有物件
                        {
                            steelParts[index].Number = number;
                            ...
                            steel.Number = steelParts[index].Count;//零件數量
                            steel.Length = steelParts[index].Length;//寫入長度
