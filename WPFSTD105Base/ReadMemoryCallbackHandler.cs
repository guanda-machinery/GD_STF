...
namespace WPFSTD105
{
    ...
    public class ReadMemoryCallbackHandler : IReadMemoryDuplexCallback
    {
        ...
        /// </summary>
        public static ManualResetEvent HostResetEvent = new ManualResetEvent(false);
        /// <summary>
        /// 控制聆聽三軸追隨座標位置請求的執行緒，暫停工作 or 執行工作。
        /// </summary>
        public static ManualResetEvent LocationResetEvent = new ManualResetEvent(false);
        /// <summary>
        /// 控制聆聽功能面板請求的執行緒，暫停工作 or 執行工作。
        /// </summary>
        public static ManualResetEvent PanelResetEvent = new ManualResetEvent(false);
        /// <summary>
        ...
        {
            ApplicationViewModel.AccountNumber.Account =new string(login.UID).TrimEnd();
            ApplicationViewModel.AccountNumber.CodeName =new string(login.Code).TrimEnd();
            ApplicationViewModel.AccountNumber.PasswordText =new string(login.Passwpord).TrimEnd();
        ...
        {
            CommonViewModel.MainAxisLocation = axisLocation;
            LocationResetEvent.Set();
        }
        /// <summary>
        ...
        public void SendPanel(PanelButton panelButton)
        {
            ...
            catch (TimeoutException ex)
            {
                ...
                //ReadDuplexMemory.State = CommunicationState.Faulted
                if (TimeoutNumber < 10)
                {
                    TimeoutNumber++;
                    return;
        ...
        public void SendDrills(WorkMaterial[] workMaterials)
        {
            throw new NotImplementedException();
        }

        public void SendIndex(ushort[] index)
        {
            throw new NotImplementedException();
        }
