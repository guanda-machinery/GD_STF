...
namespace WPFSTD105
{
    ...
    public class PanelListening : AbsListening
    {
        ...
        public bool IsReplacePage(ApplicationPage current)
        {
            ...
            {
                MessageBoxResult messageBoxResult = MessageBox.Show($"是否確定離開此頁面，離開後將不儲存相關設定。", "通知", MessageBoxButton.YesNo, MessageBoxImage.Error, MessageBoxResult.None, MessageBoxOptions.ServiceNotification);
        ...
        private void AxisMode(ref PanelButton panelButton)
        {
            ...
            {
                if (SLPEMS() || !IsManualMode() || !PromptMainAxisMode(panelButton.MainAxisMode)) //如果 按下急停 || 不是手動模式 || 沒有按下主軸模式
                {
                    ClearAxisMode(ref panelButton);
                }
            }
        }
        /// <summary>
