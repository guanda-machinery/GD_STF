...
namespace WPFSTD105
{
    ...
    public static class CodesysIIS
    {
        ...
        public static void Open()
        {
            try
            {
                ...
                if (!host.PCOpen)
                {
                    ...
                    mecOptional.AutoDrill = Optional.Default.DrillAuto;
                    mecOptional.EntranceTraverseNumber = Optional.Default.EntranceTraverseNumber; //入口橫移料架選配數量
                    mecOptional.ExportTraverseNumber = Optional.Default.ExportTraverseNumber; //出口橫移料架選配數量
                    mecOptional.StepAsideCount = 2;
                    host.Deserialize = ArmFeedOriginToOutUpRoll.Deserialize;//通知 Codesys 反序列化
                    host.PCOpen = true;

                    CodesysMemor.Create(host, ArmFeedOriginToOutUpRoll.Company);//創建共享記憶體
                    ...
                    PanelButton initialization = new PanelButton() //初始化
                    {
                        ...
                        HandSpeed = ArmFeedOriginToOutUpRoll.HandSpeed,
                    };
                    WriteCodesysMemor.SetDrillWarehouse(SetPosition(ArmFeedOriginToOutUpRoll.DrillWarehouse));//寫入刀庫資料
        ...
        private static void DrillWarehouseInitialization(MecOptional mecOptional)
        {
            ...
            ArmFeedOriginToOutUpRoll.DrillWarehouse = SetPosition(DrillWarehouse.Initialization(mecOptional));
            //判定是否無刀庫版本
            if (mecOptional.LeftExport != true)
                ArmFeedOriginToOutUpRoll.DrillWarehouse.LeftExport[0].IsCurrent = true;
            if (mecOptional.RightExport != true)
                ArmFeedOriginToOutUpRoll.DrillWarehouse.RightExport[0].IsCurrent = true;
            if (mecOptional.Middle != true)
                ArmFeedOriginToOutUpRoll.DrillWarehouse.Middle[0].IsCurrent = true;
            ArmFeedOriginToOutUpRoll.Save();
        }
    }
}
