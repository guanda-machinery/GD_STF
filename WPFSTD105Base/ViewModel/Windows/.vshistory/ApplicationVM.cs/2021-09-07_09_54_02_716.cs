...
namespace WPFSTD105
{
    ...
    public class ApplicationVM : WPFBase.BaseViewModel
    {
        ...
        public static bool IsModel(string modelPath)
        {
            ...
            if (Directory.Exists(DirectoryNc())) // NC 資料夾
                if (Directory.Exists(DirectoryPorfile())) //斷面規格目錄
                    if (File.Exists(FileProjectProperty()))//專案參數
                        if (Directory.Exists(DirectoryMaterial()))//素材3D序列化目錄
                                ...
                                if (Directory.Exists(FileSteelAssembly()))//構件資料目錄
                                    if (Directory.Exists(DirectorySteelPart()))//零件資料序列化的目錄
                                        if (Directory.Exists(DirectorySteelBolts()))//螺栓資料序列化的目錄
                                            if (Directory.Exists(FileProfileList()))//有使用的斷面規格資料表路徑
                                                if (Directory.Exists(FileMaterial()))//有使用的斷面規格資料表路徑
                                                    return true;
