    ...
    public class ApplicationVM : WPFBase.BaseViewModel
    {
        ...
        public static List<string> GetModelDirectory()
        {
            ...
            foreach (var model in Directory.GetDirectories(Properties.SofSetting.Default.LoadPath))
            {
                ...
                if (Directory.Exists($@"{model}\{name}\{ModelPath.Nc}")) // NC 資料夾
                    if (Directory.Exists($@"{model}\{name}\\{ModelPath.Profile}")) //斷面規格目錄
                        if (File.Exists($@"{model}\{name}\\{ModelPath.ProjectProperty}"))//專案參數
                            if (Directory.Exists($@"{model}\{name}\{ModelPath.DevMaterial}"))//素材3D序列化目錄
                                if (Directory.Exists($@"{model}\{name}\{ModelPath.Dev_Part}"))//單零件的3D圖形目錄
                                    if (Directory.Exists($@"{model}\{name}\{ModelPath.SteelAssembly}"))//構件資料目錄
                                        if (Directory.Exists($@"{model}\{name}\{ModelPath.SteelPart}"))//零件資料序列化的目錄
                                            if (Directory.Exists($@"{model}\{name}\{ModelPath.SteelBolts}"))//螺栓資料序列化的目錄
                                                if (File.Exists(FileProfileList($@"{model}\{name}")))//螺栓資料序列化的目錄
                                                    if (File.Exists($@"{model}\{name}\{ModelPath.Material}"))//模型材質文件
