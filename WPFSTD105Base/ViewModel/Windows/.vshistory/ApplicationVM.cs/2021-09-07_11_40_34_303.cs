    ...
    public class ApplicationVM : WPFBase.BaseViewModel
    {
        ...
        public static List<string> GetModelDirectory()
        {
            ...
            foreach (var model in Directory.GetDirectories(Properties.SofSetting.Default.LoadPath))
            {
                ...
                if (Directory.Exists($@"{DirectoryModel()}\{ModelPath.Nc}")) // NC 資料夾
                    if (Directory.Exists($"{DirectoryModel()}\\{ModelPath.Profile}")) //斷面規格目錄
                        if (File.Exists($"{DirectoryModel()}\\{ModelPath.ProjectProperty}"))//專案參數
                            if (Directory.Exists($@"{DirectoryModel()}\{ModelPath.DevMaterial}"))//素材3D序列化目錄
                                if (Directory.Exists(DirectoryDevPart()))//單零件的3D圖形目錄
                                    if (Directory.Exists(FileSteelAssembly()))//構件資料目錄
                                        if (Directory.Exists(DirectorySteelPart()))//零件資料序列化的目錄
                                            if (Directory.Exists(DirectorySteelBolts()))//螺栓資料序列化的目錄
                                                if (Directory.Exists(FileProfileList()))//斷面規格
                                                    if (Directory.Exists(FileMaterial()))//模型材質文件
