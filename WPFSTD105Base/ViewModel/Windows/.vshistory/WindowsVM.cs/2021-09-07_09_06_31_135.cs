        ...
        */
        ...
        private WPFBase.RelayParameterizedCommand OutProjectName()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                ...
                else
                {
                    ...
                    File.Copy($@"Mater.lis", $@"{ApplicationVM.ModelFileMaterial()}");//複製材質到模型內
        ...
        public WPFBase.RelayParameterizedCommand SaveAs()
        {
            return new WPFBase.RelayParameterizedCommand(el =>
            {
                ...
                Properties.SofSetting.Default.LoadPath = vm.ResultPath;//設定載入路徑
                Properties.SofSetting.Default.Save();//存取設定的路徑
                ApplicationViewModel.ProjectName = ApplicationViewModel.ProjectProperty.Name; //寫入專案名稱

                CopyDir(ApplicationVM.DirectoryModel(), $@"{ApplicationVM.DirectoryModel()}");//複製文件
