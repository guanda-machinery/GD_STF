    ...
    public class WindowsVM : STDBaseWindowView
    {
        ...
        public int SelectProject
        {
            ...
            set
            {
                ...
                if (value != -1)
                {
                    ...
                    string path = ApplicationVM.FileProjectProperty($@"{Properties.SofSetting.Default.LoadPath}\{ApplicationViewModel.ProjectList[value]}"); //指定要反序列的路徑
                    ApplicationViewModel.ProjectProperty = ser.GetProjectProperty(path);//反序列化模型屬性
                }
            }
        }
        #region 命令
        ...
        */
        ...
        protected override WPFBase.RelayParameterizedCommand OutProjectName()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                this.NewProjectControl =ApplicationVM.CreateModel((FolderBrowserDialogViewModel)e); //創建模型
            });
        }
        ///// <summary>
        ///// 修改專案屬性
        ...
        private WPFBase.RelayCommand WatchProject()
        {
            return new WPFBase.RelayCommand(() =>
            {
                ApplicationViewModel.ProjectList = new ObservableCollection<string>(ApplicationVM.GetModelDirectory(Properties.SofSetting.Default.LoadPath));
