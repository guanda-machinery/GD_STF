...
namespace WPFSTD105
{
    ...
    public class ImportNCFilesVM : WPFWindowsBase.BaseViewModel, IProjectProperty
    {
        ...
        /// 開啟屬性設定
        /// </summary>
        public ICommand PropertySettingsCommand { get; set; }
        private WPFWindowsBase.RelayCommand PropertySettings()
        {
            return new WPFWindowsBase.RelayCommand(() =>
            {
                PopupWindows popup = OfficeViewModel.PopupCurrentPage;
                if (popup == PopupWindows.PropertySettings)
                {
                    OfficeViewModel.PopupCurrentPage = PopupWindows.FirstPage;
                }
                OfficeViewModel.PopupCurrentPage = PopupWindows.PropertySettings;
            });
        }
