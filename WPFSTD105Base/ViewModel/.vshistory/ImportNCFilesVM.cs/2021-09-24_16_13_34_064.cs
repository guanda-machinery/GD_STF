                    ...
                    if (loadBomResult) //載入成功
                    {
                        ...
                        foreach (var el in teklaHtemlFactory.KeyValuePairs)//逐步存取序列化物件
                        {
                            ...
                            if (el.Value[0].GetType() == typeof(SteelPart))
                            {
                                ...
                                int index = CommonViewModel.ProjectProperty.BomProperties.FindIndex(e => e.Type == steel.Type); //查看是否有相同的斷面規格在報表屬性設定檔內
                                if (index != -1) //不再報表屬性內
                                {
                                    CommonViewModel.ProjectProperty.BomProperties.Add(new BomProperty() { Type = steel.Type });//加入到列表內
                                }
