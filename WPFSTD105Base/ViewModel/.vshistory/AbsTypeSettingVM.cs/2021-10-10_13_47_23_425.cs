...
namespace WPFSTD105
{
    ...
    public abstract class AbsTypeSettingVM : WPFBase.BaseViewModel
    {
        ...
        public AbsTypeSettingVM()
        {
            ...
            ObservableCollection<SteelAssembly.UnfoldSteelAssembly> unfoldSteelAssemblies = new ObservableCollection<SteelAssembly.UnfoldSteelAssembly>();
            ser.GetGZipAssemblies().ForEach(el =>
            {
                unfoldSteelAssemblies.AddRange(el.GetUnfold());
            });//模型內構件列表
            ...
            foreach (var profile in ser.GetProfile()) //逐步展開斷面規格
            {
                ...
                foreach (var item in buffer) //逐步展開相同零件 id or match
                {
                    
                }
                //SteelParts.AddRange(buffer); //加入到物件料表內
