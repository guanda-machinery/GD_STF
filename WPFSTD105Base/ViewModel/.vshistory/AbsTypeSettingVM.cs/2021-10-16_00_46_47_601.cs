                ...
                for (int i = 0; i < profiles.Count; i++)
                {
                    ...
                    fixed (SinglePart* pUse = usePart, pWeights = weightsPatr, pAverage = averagePart, pmUse = multipleusePart)//指標
                    {
                        ...
                        for (int q = 0; q < _[minIndex].Count; q++)
                        {
                            ...
                            MaterialDataViews[MaterialDataViews.Count - 1].Lengths.AddRange(matchSetting.MainLengths);
                            MaterialDataViews[MaterialDataViews.Count - 1].Lengths.AddRange(matchSetting.SecondaryLengths);
