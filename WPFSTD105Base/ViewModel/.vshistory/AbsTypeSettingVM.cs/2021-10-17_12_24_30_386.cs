                ...
                for (int i = 0; i < profiles.Count; i++)
                {
                    ...
                    fixed (SinglePart* pUse = usePart, pWeights = weightsPatr, pAverage = averagePart, pmUse = multipleusePart)//指標
                    {
                        ...
                        for (int q = 0; q < _[minIndex].Count; q++)
                        {
                            ...
                            MaterialDataViews[MaterialDataViews.Count - 1].LengthIndex = matchSetting.MainLengths.FindIndex(el => _[minIndex][q].Length == el);
                        ...
                        //foreach (var father in _[minIndex].GroupLength()) //逐步展開配料結果的 key 值
                        //{
                        //    foreach (var child in father) //逐步展開
                        //    {
                        //        MaterialDataViews.Add(new MaterialDataView
                        //        {
