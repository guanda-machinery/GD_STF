    ...
    public abstract class AbsTypeSettingVM : WPFBase.BaseViewModel
    {
        ...
        private unsafe WPFBase.RelayParameterizedCommand Auto()
        {
            return new WPFBase.RelayParameterizedCommand(obj =>
            {
                ...
                MatchSetting.AddRange(matchSetting.MainLengths);
                MatchSetting.AddRange(matchSetting.SecondaryLengths);
                ...
                for (int i = 0; i < profiles.Count; i++)
                {
                    ...
                    fixed (SinglePart* pUse = usePart, pWeights = weightsPatr, pAverage = averagePart, pmUse = multipleusePart)//指標
                    {
                        ...
                        for (int q = 0; q < _[minIndex].Count; q++)
                        {
                            ...
                            MaterialDataViews[MaterialDataViews.Count - 1].LengthList.AddRange(matchSetting.MainLengths);
                            MaterialDataViews[MaterialDataViews.Count - 1].LengthList.AddRange(matchSetting.SecondaryLengths);
                            MaterialDataViews[MaterialDataViews.Count - 1].LengthIndex = MaterialDataViews[MaterialDataViews.Count - 1].LengthList.FindIndex(el => _[minIndex][q].Length - matchSetting.EndCut - matchSetting.StartCut == el);
        ...
        public ObservableCollection<double> MatchSetting { get; set; } = new ObservableCollection<double>();
        ///// <summary>
        ///// 過濾斷面規格選擇器
        ///// </summary>
