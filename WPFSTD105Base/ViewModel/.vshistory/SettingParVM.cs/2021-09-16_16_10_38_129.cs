...
namespace WPFSTD105.ViewModel
{
    ...
    public class SettingParVM : WPFBase.BaseViewModel
    {
        ...
        public void UpDateDrill()
        {
            ...
            //選擇中軸刀庫
            if (IsSelectdMiddle)
                UnusedSelected = new ObservableCollection<DrillSettingVM>(GetDrillSetting(_DrillWarehouse.Middle));
            //選擇左軸入料口刀庫
            else if (IsSelectdLeftEntrance)
                UnusedSelected = new ObservableCollection<DrillSettingVM>(GetDrillSetting(_DrillWarehouse.LeftEntrance));
            //選擇左軸出料口刀庫
            else if (IsSelectdLeftExport)
                UnusedSelected = new ObservableCollection<DrillSettingVM>(GetDrillSetting(_DrillWarehouse.LeftExport));
            //選擇右軸入料口刀庫
            else if (IsSelectdRightEntrance)
                UnusedSelected = new ObservableCollection<DrillSettingVM>(GetDrillSetting(_DrillWarehouse.RightEntrance));
            //選擇右軸出料口刀庫
            else if (IsSelectdRightExport)
                UnusedSelected = new ObservableCollection<DrillSettingVM>(GetDrillSetting(_DrillWarehouse.RightExport));
            //液壓油系統參數
        ...
        private WPFBase.RelayParameterizedCommand DeleteDrillBrands()
        {
            ...
            {
                IList<DataGridCellInfo> infos = (IList<DataGridCellInfo>)e;
                infos.ForEach(el => DrillBrands.Remove((DrillBrand)el.Item));
            });
        }
        /// <summary>
        /// 新增刀具設定的列表命令
        ...
        private WPFBase.RelayCommand NewDrillBrands()
        {
            return new WPFBase.RelayCommand(() =>
            {
                ...
                while (true)
                {
                    ...
                    if (index == -1)//找不到名稱
                    {
                        DrillBrands.Add(new DrillBrand { DataName = dataName });//加入新的設定檔
                        return;
                    }
                    else
                    {
        ...
        private WPFBase.RelayCommand UseSave()
        {
            return new WPFBase.RelayCommand(() =>
            {
                ...
                {
                    DrillSettingVM _dr = UseSelected[UseSelected.Count - 1];
        ...
        /// 未裝載在主軸上的刀庫
        /// </summary>
        public ObservableCollection<DrillSettingVM> UnusedSelected { get; set; }
        /// <summary>
        /// 裝載在主軸上的刀庫
        /// </summary>
        public ObservableCollection<DrillSettingVM> UseSelected { get; set; } = new ObservableCollection<DrillSettingVM>();
        /// <summary>
        /// 液壓系統參數列表
        /// </summary>
        ...
        /// <returns></returns>
        private ObservableCollection<DrillSettingVM> DrillTools(DrillSetting[] drillSettings, bool isUse)
        {
            return new ObservableCollection<DrillSettingVM>(from el in drillSettings where el.IsCurrent == isUse select new _drillSetting(el));
        }
        /// <summary>
        /// 變更刀庫設定的值
        /// </summary>
        /// <param name="drillSetting">要變更的鑽頭的資訊</param>
        private void DrillChange(DrillSettingVM drillSetting)
        {
        ...
        /// </summary>
        /// <returns></returns>
        private List<DrillSettingVM> GetDrillSetting(DrillSetting[] drillSettings)
        {
            List<DrillSettingVM> result = new List<DrillSettingVM>();
            for (int i = 0; i < drillSettings.Length; i++)
            {
        ...
        private void CancelEnabled()
        {
            if (UseSelected.Count > 0)
            {
                List<DrillSettingVM> drills = new List<DrillSettingVM>();
                DrillSettingVM _ = UseSelected[0];
                _.IsCurrent = false;
                drills.Add(_);
                UseSelected = new ObservableCollection<DrillSettingVM>(drills);
            }
        }
        /// <summary>
