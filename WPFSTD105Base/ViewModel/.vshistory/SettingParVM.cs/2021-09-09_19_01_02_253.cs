        ...
        public WPFBase.RelayCommand DeleteMaterial()
        {
            return new WPFBase.RelayCommand(() =>
            {
                if (_Materials.IndexOf(el => el.Name == SelectMaterial.Name) != -1) //判斷材質是否重複
                {
                    ...
                    if (system.FindIndex(el => el.Name == SelectMaterial.Name) != -1) //如果系統存在相同物件
                    {
                        ...
                        {
                            system.Remove(el => el.Name == SelectMaterial.Name); //刪除相同物件
        ...
        public WPFBase.RelayParameterizedCommand SaveMaterial()
        {
            return new WPFBase.RelayParameterizedCommand(par =>
            {
                if (_Materials.IndexOf(el => el.Name == SelectMaterial.Name) == -1) //判斷材質是否重複
                {
                    ...
                    std.SetMaterial(_Materials);//存入在模型設定檔
                    if (str == "系統")//如果要變更系統材質
                    {
        ...
        private WPFBase.RelayParameterizedCommand Delete()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                ...
                if (e is SteelAttr steelAttr)
                {
                    ...
                    ObservableCollection<SteelAttr> back = (ObservableCollection<SteelAttr>)backPropertyInfo.GetValue(this);//反射在背景的資料列表
                    back.Remove(el => el.Profile == steelAttr.Profile);
