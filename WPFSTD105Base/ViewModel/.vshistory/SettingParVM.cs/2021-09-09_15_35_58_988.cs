...
namespace WPFSTD105.ViewModel
{
    ...
    public class SettingParVM : WPFBase.BaseViewModel
    {
        ...
        public SettingParVM()
        {
            ...
            NewMaterialCommand = NewMaterial();
            ...
            DrillBrands.Add(new DrillBrand());
            ser.SetDrillBrands(DrillBrands);
        ...
        public WPFBase.RelayCommand DeleteMaterial()
        {
            return new WPFBase.RelayCommand(() =>
            {
                SaveAsWindowsControl = false;
                /*
                if (_Materials.IndexOf(el => el.Name == SelectMaterial.Name) == -1) //判斷材質是否重複
                {
                    _Materials.Add(SelectMaterial);
                }
                else
                {
                    MessageBox.Show($"已有存在相同名稱", "通知", MessageBoxButton.YesNo, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.ServiceNotification);
                }
                */
            });
        }
        ...
        private WPFBase.RelayCommand UpdateMaterial()
        {
            return new WPFBase.RelayCommand(() =>
            {
                SaveAsWindowsControl = false;
        ...
        public WPFBase.RelayCommand NewMaterial()
        {
            return new WPFBase.RelayCommand(() =>
            {
                SaveAsWindowsControl = false;
        ...
        public WPFBase.RelayParameterizedCommand SaveMaterial()
        {
            return new WPFBase.RelayParameterizedCommand(par =>
            {
                ...
                if (_Materials.IndexOf(el => el.Name == SelectMaterial.Name) == -1) //判斷材質是否重複
                {
                    ...
                    STDSerialization std = new STDSerialization();
                    _Materials.Add(SerializationHelper.Clone<SteelMaterial>(SelectMaterial));
                    Materials.Add(SerializationHelper.Clone<SteelMaterial>(SelectMaterial));
                    std.SetMaterial(_Materials);
                    if (str == "系統")
                    {
                        std.SetMaterial(ModelPath.Material, _Materials);
                    }
                }
                else
                {
        ...
        public WPFBase.RelayCommand Save()
        {
            return new WPFBase.RelayCommand(() =>
            {
                ...
                /*
                //TODO: 這邊要修改
                switch ((OBJETC_TYPE)_SelectType)
                {
                    case OBJETC_TYPE.RH:
                        _RH = _Save(_RH);
                        RH = new ObservableCollection<SteelAttr>(_RH.ToList());
                        SerializationHelper.SerializeBinary(_RH, $@"{ModelPath.Profile}\RH.inp");
                        break;
                    case OBJETC_TYPE.CH:
                        _CH = _Save(_CH);
                        CH = new ObservableCollection<SteelAttr>(_CH.ToList());
                        SerializationHelper.SerializeBinary(_CH, $@"{ModelPath.Profile}\CH.inp");
                        break;
                    case OBJETC_TYPE.L:
                        _L = _Save(_L);
                        L = new ObservableCollection<SteelAttr>(_L.ToList());
                        SerializationHelper.SerializeBinary(_L, $@"{ModelPath.Profile}\L.inp");
                        break;
                    case OBJETC_TYPE.BOX:
                        _BOX = _Save(_BOX);
                        BOX = new ObservableCollection<SteelAttr>(_BOX.ToList());
                        SerializationHelper.SerializeBinary(_BOX, $@"{ModelPath.Profile}\BOX.inp");
                        break;
                    case OBJETC_TYPE.BH:
                        _BH = _Save(_BH);
                        SerializationHelper.SerializeBinary(_BH, $@"{ModelPath.Profile}\BH.inp");
                        break;
                    default:
                        MessageBox.Show("新增斷面規格失敗。\n請選擇斷面規格類型。", "通知", MessageBoxButton.OK, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.ServiceNotification);
                        break;
                }*/
            });
        }
        ...
        public WPFBase.RelayParameterizedCommand SaveProfile()
        {
            return new WPFBase.RelayParameterizedCommand(el =>
            {
                ...
                object getValue = (ObservableCollection<SteelAttr>)showPropertyInfo.GetValue(this); //反射在背景的資料列表
                ...
                showPropertyInfo.SetValue(this, new ObservableCollection<SteelAttr>(value.ToList()));//存取資料到顯示畫面的列表
                SerializationHelper.SerializeBinary(value, $@"{ApplicationVM.DirectoryModel()}\{ModelPath.Profile}\{strType}.inp");
                if (str == "系統")
                {
                    SerializationHelper.SerializeBinary(value, $@"{ModelPath.Profile}\{strType}.inp");
                }
            });
        }
        ...
        /// </summary>
        /// <returns></returns>
        public WPFBase.RelayCommand UpdateTree()
        {
            return new WPFBase.RelayCommand(() =>
            {
                DisplaySavePosition = true;
                ...
                switch ((OBJETC_TYPE)_SelectType)
                {
                    ...
                    case OBJETC_TYPE.BH:
                        ...
                        break;
                    default:
                        MessageBox.Show("更新斷面規格失敗。", "通知", MessageBoxButton.OK, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.ServiceNotification);
                        break;
                }
                */
            });
        ...
        private WPFBase.RelayCommand Delete()
        {
            return new WPFBase.RelayCommand(() =>
            {
                DisplaySavePosition = true;
                /*
                if (SelectSteelAtte != null)
                {
                    switch (SelectSteelAtte.Type)
                    {
                        case OBJETC_TYPE.RH:
                            _RH.Remove(SelectSteelAtte);
                            RH = new ObservableCollection<SteelAttr>(_RH.ToList());
                            SerializationHelper.SerializeBinary(_RH, $@"{ModelPath.Profile}\RH.inp");
                            break;
                        case OBJETC_TYPE.CH:
                            _CH.Remove(SelectSteelAtte);
                            CH = new ObservableCollection<SteelAttr>(_CH.ToList());
                            SerializationHelper.SerializeBinary(_CH, $@"{ModelPath.Profile}\RH.inp");
                            break;
                        case OBJETC_TYPE.L:
                            _L.Remove(SelectSteelAtte);
                            L = new ObservableCollection<SteelAttr>(_L.ToList());
                            SerializationHelper.SerializeBinary(_L, $@"{ModelPath.Profile}\RH.inp");
                            break;
                        case OBJETC_TYPE.BOX:
                            _BOX.Remove(SelectSteelAtte);
                            BOX = new ObservableCollection<SteelAttr>(_BOX.ToList());
                            SerializationHelper.SerializeBinary(_BOX, $@"{ModelPath.Profile}\RH.inp");
                            break;
                        case OBJETC_TYPE.BH:
                            _BH.Remove(SelectSteelAtte);
                            BH = new ObservableCollection<SteelAttr>(_BH.ToList());
                            SerializationHelper.SerializeBinary(_BH, $@"{ModelPath.Profile}\RH.inp");
                            break;
                        default:
                            MessageBox.Show("刪除斷面規格失敗。", "通知", MessageBoxButton.OK, MessageBoxImage.Exclamation, MessageBoxResult.None, MessageBoxOptions.ServiceNotification);
                            break;
                    }
                }
                else
        ...
        /// 選擇的參數檔案
        /// </summary>
        public SteelAttr SelectSteelAtte { get; set; } = new SteelAttr();
        /// <summary>
        /// 顯示Beam 2D圖
        /// </summary>
        ...
        public int MatrialIndex
        {
            ...
            set
            {
                ...
                if (value != -1)
                {
                    SelectMaterial = SerializationHelper.Clone<SteelMaterial>(_Materials[value]);
                }
            }
        }
        /// <summary>
        ...
        {
            STDSerialization ser = new STDSerialization();
            RH = SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"{ModelPath.Profile}\RH.inp");
            BH = SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"{ModelPath.Profile}\BH.inp");
            BOX = SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"{ModelPath.Profile}\BOX.inp");
            CH = SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"{ModelPath.Profile}\CH.inp");
            L = SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"{ModelPath.Profile}\L.inp");
        ...
        private int _SelectType { get; set; } = 0;
        #endregion
