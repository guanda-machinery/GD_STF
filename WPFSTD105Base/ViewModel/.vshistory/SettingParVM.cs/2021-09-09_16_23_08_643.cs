    ...
    public class SettingParVM : WPFBase.BaseViewModel
    {
        ...
        public WPFBase.RelayCommand DeleteMaterial()
        {
            return new WPFBase.RelayCommand(() =>
            {
                if (_Materials.IndexOf(el => el.Name == SelectMaterial.Name) == -1) //判斷材質是否重複
                {
                    ...
                    MessageBoxResult message = MessageBox.Show($"是否連同系統的也一並刪除?", "通知", MessageBoxButton.YesNo, MessageBoxImage.Information, MessageBoxResult.None, MessageBoxOptions.ServiceNotification);
                    _Materials.Remove(el => el.Name == SelectMaterial.Name);
                    Materials = SerializationHelper.Clone(_Materials); //複製物件
                    std.SetMaterial(_Materials);
                    if (message == MessageBoxResult.Yes)//如果確定要變更系統
                    {
                        std.SetMaterial(ModelPath.Material, _Materials);//變更系統內設定的材質
                    }
                }
            });
        }
        ...
        public WPFBase.RelayParameterizedCommand SaveMaterial()
        {
            return new WPFBase.RelayParameterizedCommand(par =>
            {
                if (_Materials.IndexOf(el => el.Name == SelectMaterial.Name) == -1) //判斷材質是否重複
                {
                    ...
                    Materials.Add(SerializationHelper.Clone<SteelMaterial>(SelectMaterial));//加入到顯示畫面
                    std.SetMaterial(_Materials);
                    if (str == "系統")//如果要變更系統材質
                    {
                        std.SetMaterial(ModelPath.Material, _Materials);//變更系統內設定的材質
        ...
        public WPFBase.RelayParameterizedCommand SaveProfile()
        {
            return new WPFBase.RelayParameterizedCommand(el =>
            {
                ...
                if (str == "系統")//如果確定要變更系統
                {
                    SerializationHelper.SerializeBinary(value, $@"{ModelPath.Profile}\{strType}.inp"); //變更系統斷面規格檔案
                }
                SaveAsWindowsControl = true;
            });
        }
