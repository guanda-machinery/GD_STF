...
namespace WPFSTD105.ViewModel
{
    ...
    public class SettingParVM : WPFBase.BaseViewModel
    {
        ...
        public WPFBase.RelayParameterizedCommand DeleteMaterial()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                if (e is SteelMaterial material) //判斷材質是否重複
                {
                    ...
                    Materials.Remove(el => el.Name == material.Name);
                    std.SetMaterial(Materials);
        ...
        private WPFBase.RelayParameterizedCommand UpdateMaterial()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                ...
                Materials[MatrialIndex] = SerializationHelper.Clone(SelectMaterial); //變更顯示物件
        ...
        public WPFBase.RelayParameterizedCommand UpdateTree()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                ...
                if (e is SteelAttr steelAttr)
                {
                    ...
                    PropertyInfo propertyInfo = this.GetType().GetProperty(strType);//反射在畫面顯示的資料列表

                    ObservableCollection<SteelAttr> lsit = (ObservableCollection<SteelAttr>)propertyInfo.GetValue(this);//反射在背景的資料列表
                    lsit.Remove(el => el.Profile == steelAttr.Profile);
        ...
        private WPFBase.RelayParameterizedCommand Delete()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                ...
                if (e is SteelAttr steelAttr)
                {
                    ...
                    ObservableCollection<SteelAttr> list = (ObservableCollection<SteelAttr>)propertyInfo.GetValue(this);//反射在背景的資料列表
                    list.Remove(el => el.Profile == steelAttr.Profile);
                    ObservableCollection<SteelAttr> system = SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"{ModelPath.Profile}\{strType}.inp");//系統內設定的斷面規格
                    ...
                    //ObservableCollection<SteelAttr> back = (ObservableCollection<SteelAttr>)backPropertyInfo.GetValue(this);//反射在背景的資料列表
                    //back.Remove(el => el.Profile == steelAttr.Profile);
                    //showPropertyInfo.SetValue(this, SerializationHelper.Clone(back));
                    //ObservableCollection<SteelAttr> system = SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"{ModelPath.Profile}\{strType}.inp");//系統內設定的斷面規格
