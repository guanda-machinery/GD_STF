        ...
        public WPFBase.RelayParameterizedCommand UpdateTree()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                ...
                if (e is SteelAttr steelAttr)
                {
                    ...
                    int backIndex = back.IndexOf(steelAttr);//背景列表位置 
                    back[backIndex] = steelAttr; //變更模型資料
                    showPropertyInfo.SetValue(this, back);//賦予到顯示列表
                    ...
                    if (system.IndexOf(el => el.Profile == steelAttr.Profile) != -1)//如果系統存在相同物件
                    {
                        ...
                        if (message == MessageBoxResult.Yes)//如果確定要變更系統
                        {
                            int systemIndex = system.IndexOf(steelAttr);//選擇系統列表的位置
                            system[systemIndex] = SerializationHelper.Clone(steelAttr);//變更模型資料
