...
namespace WPFSTD105.ViewModel
{
    ...
    public class SettingParVM : WPFBase.BaseViewModel
    {
        ...
        public void UpDateDrill()
        {
            ...
            Default.DrillWarehouse = _DrillWarehouse;//將在記憶體內的刀庫設定賦予軟體設定檔
        ...
        private WPFBase.RelayParameterizedCommand DeleteDrillBrands()
        {
            return new WPFBase.RelayParameterizedCommand(e =>
            {
                ...
                infos.ForEach(el => SofSetting.Default.DrillBrands.Remove((DrillBrand)el.Item));
            });
        }
        /// <summary>
        /// 新增刀具設定的列表命令
        ...
        private WPFBase.RelayCommand NewDrillBrands()
        {
            return new WPFBase.RelayCommand(() =>
            {
                ...
                while (true)
                {
                    ...
                    int index = SofSetting.Default.DrillBrands.IndexOf(el => el.DataName == dataName); //查詢相同名稱
                    if (index == -1)//找不到名稱
                    {
                        SofSetting.Default.DrillBrands.Add(new DrillBrand { DataName = dataName });//加入新的設定檔
                        return;
                    }
                    else
                    {
        ...
        #region VM類型
        [Serializable]
        public class _drillSetting : WPFBase.BaseViewModel
        {
            private double dia;
            ...
            public double Dia { get => SofSetting.Default.DrillBrands[SettingIndex].Dia; set => dia = value; }
            public double Rpm { get => SofSetting.Default.DrillBrands[SettingIndex].Rpm; set => rpm = value; }
            public double FeedQuantity { get => SofSetting.Default.DrillBrands[SettingIndex].FeedQuantity; set => feedQuantity = value; }
            public double Length { get => SofSetting.Default.DrillBrands[SettingIndex].HolderLength + SofSetting.Default.DrillBrands[SettingIndex].DrillLength; set => length = value; }
            public short Limit { get; set; }
            public string Name { get => SofSetting.Default.DrillBrands[SettingIndex].Name; set => name = value; }
            public double f { get => SofSetting.Default.DrillBrands[SettingIndex].f; set => f1 = value; }
            public ObservableCollection<string> DataNames { get ;  set; }
            public _drillSetting(DrillSetting drillSetting)
            {
                Index = drillSetting.Index;
                SettingIndex = drillSetting.SettingIndex;
                DrillTypeIndex = (int)drillSetting.DrillType;
                Limit = drillSetting.Limit;
                DataNames = new ObservableCollection<string>(SofSetting.Default.DrillBrands.Select(el => el.DataName));
            }
        }
        /// <summary>
        /// 接點
