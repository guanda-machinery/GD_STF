    ...
    public class ObSettingVM : WPFBase.BaseViewModel
    {
            ...
            set
            {
                try
                {
                    ...
                    switch (TYPE)
                    {
                        ...
                        case OBJETC_TYPE.BH:
                            ProfileList = SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"Profile\{ (TYPE).ToString()}.inp");
                            if (File.Exists(ApplicationVM.DirectoryPorfile()))
                            {
                                ProfileList.AddRange(SerializationHelper.Deserialize<ObservableCollection<SteelAttr>>($@"{ApplicationVM.DirectoryPorfile()}\{ (TYPE).ToString()}.inp"));
                            }
                            break;
                        default:
        ...
        public int MaterialIndex
        {
            ...
            set
            {
                ...
                SteelAttr.Material = Materials[value];
#if DEBUG
        ...
        public void SaveDataCorrespond()
        {
            ...
            if (save)
            {
                ...
                SerializationHelper.SerializeBinary(DataCorrespond, ApplicationVM.PartList());
                AddNode(data);
            }
        }
        /// <summary>
        ...
        public GroupBoltsAttr GetGroupBoltsAttr()
        {
            ...
            if (CheckDia == true)
            {
                ...
                Boltsbuffer.Mode = (GD_STD.Enum.AXIS_MODE)AxisModeType;
            }
            //水平螺栓間距
            if (CheckX)
            {
        ...
        public void WriteSteelAttr(SteelAttr steelAttr)
        {
            ...
            MaterialIndex = Materials.IndexOf(steelAttr.Material);
        ...
        public ObSettingVM()
        {
            ...
            //如果模型有材質設定
            if (!File.Exists(ApplicationVM.ModelFileMaterial()))
            {
                Materials.AddRange(SerializationHelper.GZipDeserialize<ObservableCollection<string>>(ApplicationVM.ModelFileMaterial())); //材質序列化檔案
            }

            ObservableCollection<string> _ = SerializationHelper.GZipDeserialize<ObservableCollection<string>>(@"Mater.lis"); //材質序列化檔案
            ...
            DataCorrespond = SerializationHelper.Deserialize<ObservableCollection<DataCorrespond>>(ApplicationVM.PartList());
