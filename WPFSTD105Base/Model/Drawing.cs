...
namespace WPFSTD105
{
    ...
    partial class ModelExt
    {
        ...
        /// <returns></returns>
        private Point3D[] GetScreenVertices(IList<Point3D> vertices)
        {
            Point3D[] result = new Point3D[vertices.Count]; //屏幕點的數量
            for (int i = 0; i < vertices.Count; i++)
            {
                result[i] = WorldToScreen(vertices[i]);//將世界坐標映射到屏幕坐標。
            }
            return result;
        }
        /// <summary>
        ...
        private void DrawInteractiveLines()
        {
            ...
            Point3D[] screenPts = GetScreenVertices(points);
