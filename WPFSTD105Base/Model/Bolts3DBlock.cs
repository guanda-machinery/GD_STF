...
namespace WPFSTD105.Model
{
    ...
    public class Bolts3DBlock : Block, IBolts
    {
        ...
        /// <returns></returns>
        //public static bool CheckWorkingRange(GroupBoltsAttr gba,OBJECT_TYPE type, double checkValue, List<double> workingRange) AXIS_MODE Mode, FACE face
                   ...
                   ((GroupBoltsAttr)(x.EntityData)).GUID.Value.ToString() == boltCheck[i].Block.Name))
                {
                    ...
                    if (CheckWorkingRange(bolt.Mode, bolt.Face, type, checkValue, list))
                    {
                        if (bolt.Mode != AXIS_MODE.POINT && bolt.Mode != AXIS_MODE.HypotenusePOINT)
                        {
                            ...
                            //rotate = true;
                            if (bolt.Face == FACE.FRONT || bolt.Face == FACE.BACK)
                            {
                                rotate = true;
                            }
                            if (rotate)
