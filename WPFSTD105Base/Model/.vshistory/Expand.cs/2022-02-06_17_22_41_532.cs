...
namespace WPFSTD105.Model
{
    ...
    public static class Expand
    {
        ...
        public static void AssemblyPart(this devDept.Eyeshot.Model model, string materialNumber)
        {
            ...
            List<double> position = new List<double>(); //第一次移動的座標(只記錄 end)，匹配 finish 位置
            ...
            for (int i = 0; i < viewIndex.Count; i++)
            {
                ...
                if (finish.Contains(viewIndex[i]))//如果有匯入到 3D Model
                {
                    ...
                    ent.Where(el => el.GroupIndex == finish.IndexOf(viewIndex[i])).ForEach(el =>
                    {
                        ...
                        entity.GroupIndex = i;
                        double x = end - position[posIndex];
                    ...
                    if (parts[viewIndex[i]].GUID.ToString() != "") //有 NC 檔
                    {
                        ...
                        model.Entities.ForEach(el =>
                        {
                            model.Blocks[((BlockReference)el).BlockName].Entities.ForEach(entity =>
                            {
                                ...
                                {
                                    entity.Color = ColorTranslator.FromHtml(Properties.SofSetting.Default.Ingredient1);
                                }
                            });
                            el.GroupIndex = i;
                            el.Translate(end, 0);
            ...
            DrawCutMesh(parts[0], model, end - material.Cut, material.LengthList[material.LengthIndex] + material.StartCut + material.EndCut, ent);
