...
namespace WPFSTD105.Model
{
    ...
    public class Bolts3DBlock : Block, IBolts
    {
        ...
        {
            List<Bolt> resultY = new List<Bolt>();//Y向螺栓結果(3D實體)，要給X向複製用
            //如果是打點改直徑
            if (this.Info.Mode == AXIS_MODE.POINT)
            {
                this.Info.Dia = 10;
            }
            resultY.Add(Mesh.CreateCylinder<Bolt>(this.Info.Dia * 0.5, this.Info.t + h * 2, 22));//創建一個圓柱體
            //如果是打點改變顏色
            if (this.Info.Mode == AXIS_MODE.POINT)
            {
            ...
            {
                //複製物件
                Bolt bolt = (Bolt)resultY[i - 1].Clone();
        ...
        public Bolts3DBlock AddBolts(GroupBoltsAttr attr, devDept.Eyeshot.Model model)
        {
            Bolts3DBlock result = new Bolts3DBlock(attr); //產生孔位圖塊
            result.CreateBolts();//創建孔位群組
            model.Blocks.Add(result);//加入孔位群組圖塊到模型
            BlockReference blockReference = new BlockReference(0, 0, 0, result.Name, 1, 1, 1, 0);//產生孔位群組參考圖塊
            blockReference.EntityData = result.Info;
