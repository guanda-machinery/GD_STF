...
namespace WPFSTD105
{
    ...
    public partial class ModelExt : devDept.Eyeshot.Model
    {
        ...
        protected override void OnMouseDown(MouseButtonEventArgs e)
        {
            ...
            if (ActionMode == actionType.None && e.ChangedButton == System.Windows.Input.MouseButton.Left)
            {
                ...
                else if (drawingLinearDim && points.Count == 3)
                {
                    ...
                    this.Entities[0].Selectable = false;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            {
                Entities[0].Selectable = true;
                this.snapPoint = null;
                snapPoints = GetSnapPoints(mouseLocation);
                Entities[0].Selectable = false;
            }
        ...
        {
            ObSettingVM _ = (ObSettingVM)DataContext;
            if (e.Key == Key.Delete)
            {
                ...
                if (_.Select3DItem.Count > 0 && CurrentBlockReference == null) //判斷是否有選擇到物件
                {
                    List<Entity> sele3D = new List<Entity>(), sele2D = new List<Entity>();
                    _.Select3DItem.ForEach(el => sele3D.Add((BlockReference)el.Item));
                    _.Select2DItem.ForEach(el => sele2D.Add((BlockReference)el.Item));
                    _.Reductions.Add(new Reduction() //加入到垃圾桶內
                    {
                    ...
                    if (Name == "model")
                    {
                        _.Select2DItem.ForEach(el => Secondary.Entities.Remove((BlockReference)el.Item));
                    }
                    else
                    {
                        _.Select3DItem.ForEach(el => Secondary.Entities.Remove((BlockReference)el.Item));
                    }
                }
                else //編輯模式
                {
                    if (this.Name == "model")
                    {
                        ...
                        _.tem2DRecycle.ForEach(el => Secondary.Entities.Remove(el));
                    }
                    else
                    {
                        drawingBlock = this.CurrentBlockReference;
                        modelBlock = Secondary.CurrentBlockReference;
                        _.tem3DRecycle.ForEach(el => Secondary.Entities.Remove(el));
                    }
                    _.Reductions.Add(new Reduction() //加入到垃圾桶內
                    {
                        SelectReference = modelBlock,
                        Recycle = new List<List<Entity>>() { _.tem3DRecycle.ToList() },
                        User = new List<ACTION_USER>() { ACTION_USER.DELETE }
                    }, new Reduction() //加入到垃圾桶內
                    {
                        SelectReference = drawingBlock,
                        Recycle = new List<List<Entity>>() { _.tem2DRecycle.ToList() },
                        User = new List<ACTION_USER>() { ACTION_USER.DELETE }
                    });
                }
                ...
                //清空選擇物件
                _.Select2DItem.Clear();
                _.Select3DItem.Clear();
                //清空圖塊內物件
                _.tem3DRecycle.Clear();
                _.tem3DRecycle.Clear();
            }
