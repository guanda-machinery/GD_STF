...
#pragma warning disable CS1998 // 這個非同步方法缺少 'await' 運算子，因此將以同步方式執行。請考慮使用 'await' 運算子等候未封鎖的應用程式開發介面呼叫，或使用 'await Task.Run(...)' 在背景執行緒上執行 CPU-bound 工作。
        static async Task Main(string[] args)
#pragma warning restore CS1998 // 這個非同步方法缺少 'await' 運算子，因此將以同步方式執行。請考慮使用 'await' 運算子等候未封鎖的應用程式開發介面呼叫，或使用 'await Task.Run(...)' 在背景執行緒上執行 CPU-bound 工作。
        {
#pragma warning disable CS0219 // 已指派變數 'ip'，但是從未使用過它的值
            string ip = "";
#pragma warning restore CS0219 // 已指派變數 'ip'，但是從未使用過它的值
            try
            {
                WebClient MyWebClient = new WebClient();
                MyWebClient.Credentials = CredentialCache.DefaultCredentials;//獲取或設置用於向Internet資源的請求進行身份驗證的網路憑據

                string pageData = MyWebClient.DownloadString("http://www.net.cn/static/customercare/yourip.asp"); //從指定網站下載數據
