...
namespace ConsoleApp3
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            OpenSharedMemory();
            long po = SharedMemory.GetMemoryOffset<MonitorWork>(work, $"{nameof(MonitorWork.WorkMaterial)}[1].{nameof(WorkMaterial.Material)}");

            Enum e = MATERIAL.SM490A;
            Type underlying = Enum.GetUnderlyingType(e.GetType());
            object writeValue = Activator.CreateInstance(underlying);
            PCSharedMemory.SetValue<MonitorWork>(po, writeValue);
