...
namespace ConsoleApp3
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            work.WorkMaterial[0].Finish = ushort.MaxValue;
            work.WorkMaterial[0].Length = double.MaxValue;
            int intPtr =   Marshal.OffsetOf(typeof(WorkMaterial), nameof(WorkMaterial.Length)).ToInt32();
            //byte[] vs = work.ToByteArray();
            //work = vs.FromByteArray<MonitorWork>();
            OpenSharedMemory();
            WorkMaterial workMaterials = SharedMemory.GetValue<MonitorWork, WorkMaterial>(0, Marshal.SizeOf(typeof(WorkMaterial)));
            long po = SharedMemory.GetMemoryOffset<MonitorWork>(work, $"{nameof(MonitorWork.WorkMaterial)}[0].{nameof(WorkMaterial.Length)}");
            //Marshal.OffsetOf(typeof(WorkMaterial), "Length");
            double _ = 200d;
            Enum e = MATERIAL.SM490A;
            PCSharedMemory.SetValue<MonitorWork>(intPtr, _);
            long resultPo = SharedMemory.GetMemoryOffset<MonitorWork>(work, $"{nameof(MonitorWork.WorkMaterial)}[0]");
            WorkMaterial result = PCSharedMemory.GetValue<MonitorWork, WorkMaterial>(resultPo, Marshal.SizeOf(typeof(WorkMaterial)));
