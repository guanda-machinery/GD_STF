...
namespace ConsoleApp3
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            //Task.Run(() =>
            //{
            //    int size = Marshal.SizeOf(typeof(aaa));
            //    using (var memory = memoryMappedFile.CreateViewAccessor(0, size, MemoryMappedFileAccess.Write))
            //    {
            //        int i = 200;
            //        while (true)
            //        {
            //            byte[] buffer = i.ToByteArray();
            //            memory.WriteArray<byte>(Marshal.SizeOf(typeof(int)) * 2, buffer, 0, Marshal.SizeOf(typeof(int)));
            //        }
            //    }
            //});
            ...
            //{
            //    int size = Marshal.SizeOf(typeof(aaa));
            //    using (var memory = memoryMappedFile.CreateViewAccessor(0, size, MemoryMappedFileAccess.Read))
            //    {
            //        while (true)
            //        {
            ...
            //            aaa _ = buffer.FromByteArray<aaa>();
            //            Console.WriteLine($"[1]={_.Array[1]},[2]={_.Array[2]}");
            //            Thread.Sleep(1000);
            //        }
            //    }
            //});
            ...
            Action write = () =>
            {
                ...
                using (var memory = charMappedFile.CreateViewAccessor(0, size, MemoryMappedFileAccess.Write))
                {
                    int i = 0;
                    while (i > 200)
                    {
                        byte[] buffer = '你'.ToByteArray();
                        memory.WriteArray<byte>(0, buffer, 0, Marshal.SizeOf(typeof(char)));
                        Thread.Sleep(1000);
                        i++;
                    }
                }
            ...
            ,
                       ...
                       {
                           int size = Marshal.SizeOf(typeof(char));
                           using (var memory = charMappedFile.CreateViewAccessor(0, size, MemoryMappedFileAccess.Read))
                           {
                               while (true)
                               {
                                   byte[] buffer = new byte[size];
                                   ...
                                   Console.WriteLine($"{Convert.ToChar(buffer[0])}");
                                   Thread.Sleep(1000);
                               }
                           }
                       };

            Task[] tasks = new Task[] { new Task(write), new Task(read, cts.Token) };
            for (int i = 0; i < tasks.Length; i++)
            {
                tasks[i].Start();
            }
            Console.Read();
            cts.CancelAfter(new TimeSpan(0, 0, 0, 0));
