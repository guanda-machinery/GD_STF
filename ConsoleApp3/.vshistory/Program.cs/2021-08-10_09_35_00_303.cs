...
namespace ConsoleApp3
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var WorkMateriald = Marshal.SizeOf(typeof(WorkMaterial));
            OpenSharedMemory();
            PCSharedMemory.SetValue<MonitorWork>(work);
            WorkMaterial workMaterials = SharedMemory.GetValue<MonitorWork, WorkMaterial>(0, Marshal.SizeOf(typeof(WorkMaterial)));
            long po = SharedMemory.GetMemoryOffset<MonitorWork>(work, $"{nameof(MonitorWork.WorkMaterial)}[2999].{nameof(WorkMaterial.Finish)}");
            Int16 _ = 200;
            Enum e = MATERIAL.SM490A;
            PCSharedMemory.SetValue<MonitorWork>(po, _);
            long resultPo = SharedMemory.GetMemoryOffset<MonitorWork>(work, $"{nameof(MonitorWork.WorkMaterial)}[2999]");
