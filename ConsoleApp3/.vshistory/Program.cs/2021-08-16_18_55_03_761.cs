...
namespace ConsoleApp3
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            long size = Marshal.OffsetOf(typeof(MonitorWork), nameof(MonitorWork.WorkMaterial)).ToInt64();
            long workMaterialSize = Marshal.SizeOf(typeof(WorkMaterial));
            for (int i = 0; i < work.Count; i++)
            {
                WorkMaterial result = WorkMaterial.Initialization();
                result.Length = i;
                ClientWrite.SetMonitorWorkOffset(result.ToByteArray(), size + workMaterialSize * i);
            }
