...
namespace ConsoleApp3
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int i = Marshal.SizeOf(typeof(WorkMaterial));
            IntPtr _ = Marshal.OffsetOf(typeof(MonitorWork), $"{nameof(MonitorWork.WorkMaterial)}[1].{nameof(WorkMaterial.Length)}");
            OpenSharedMemory();
            long setLength = SharedMemory.GetMemoryOffset<MonitorWork>(monitorWork, $"{nameof(MonitorWork.WorkMaterial)}[1].{nameof(WorkMaterial.Length)}");
            PCSharedMemory.SetValue<MonitorWork>(setLength, 200d);
            WorkMaterial workMaterial = SharedMemory.GetValue<MonitorWork, WorkMaterial>(Marshal.SizeOf(typeof(WorkMaterial)), Marshal.SizeOf(typeof(WorkMaterial)));
