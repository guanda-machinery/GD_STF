...
namespace 測試自動配料
{
    ...
    public class Tekla報表轉物件測試
    {
        ...
        private static IEnumerable<string> GetBom()
        {
           ...
           yield return @"C:\Users\User\Desktop\專案BOM表區\19公分\GUANDA_Personal_Format.csv";
        }
        private static IEnumerable<string> GetNC()
        {
            //IoC.Setup();
            yield return @"19公分";
        }
        ...
        public void 測試載入VM()
        {
            SplashScreenManager ScreenManager = SplashScreenManager.CreateWaitIndicator();
            ScreenManager.Show();
            VM(ScreenManager.ViewModel);
            ScreenManager.Close();

        }
        public void VM(DXSplashScreenViewModel vm)
        {
            int i = 0;
            while (true)
            {
        ...
        [Test, TestCaseSource("GetNC")]
        public void 測試NC轉換(string name)
        {
            SplashScreenManager ScreenManager = SplashScreenManager.CreateWaitIndicator();
            ScreenManager.Show();
            ApplicationViewModel.ProjectName = name;
            TeklaNcFactory factory = new TeklaNcFactory();
            factory.Load(ScreenManager.ViewModel);

            ScreenManager.Close();
        }
        [Test]
        public void 測試NC檔案轉換()
        {
            TeklaNcFactory teklaNcFactory = new TeklaNcFactory();
            teklaNcFactory.Load(ncPath: @"C:\Users\User\Desktop\專案BOM表區");
        }
        ...
        public void 測試報表轉換(string path)
        {
            ...
            File.Copy($@"C:\Users\User\source\repos\GD_STF\STD_105\bin\Debug\Mater.lis", $@"{ApplicationVM.FileMaterial()}");//複製材質到模型內
            ser.SetProjectProperty(new ProjectProperty() { Create = DateTime.Now }); //存取設定

            TeklaBomFactory teklaHtemlFactory = new TeklaBomFactory($@"{path}"); //報表讀取器
            ...
            ObservableCollection<string> profileList = ser.GetProfile();
