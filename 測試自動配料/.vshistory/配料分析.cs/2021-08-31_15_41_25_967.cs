...
namespace 測試自動配料
{
    ...
    public class 配料分析
    {
        ...
        public unsafe void 自動配料(string projectName)
        {
            ...
            SinglePart[] singlePart = singleParts.ToArray();

            fixed (SinglePart* ptr = singlePart)
            {
                ptr[0].Match = true;

                SinglePart part = ptr[0];
                List<Chromosome> chromosomes = new List<Chromosome>();
                while (singleParts.Count != 0)
                {

                    Population pop = new Population(matchSetting, ptr, singlePart.Length);
                    pop.Initialize();
                    Chromosome chromosome = pop.Suitable();
                    for (int i = 0; i < chromosome.DNA.Count; i++)
                    {
                        index = chromosome.DNA[i];
                        ptr[index].Match = true;
                    }
        ...
        public unsafe class Population : List<Chromosome>
        {
            ...
            public void Initialize()
            {
                ...
                {
                    this.Add(new Chromosome(lengths[i], _Match, index, _SingleParts, _Count));
                }
            }
            /// <summary>
            /// 取得最合適的物件
