        ...
        public unsafe void 完整自動配料(string projectName)
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                fixed (SinglePart* pUse = usePart)//使用率列表指標
                {
                    ...
                    Debug.WriteLine($"{uPopulation.ToString()}");
                    foreach (var item in uPopulation.GroupLength())
                    {
                        Debug.WriteLine($"長度 : {item.Key} 數量 : {item.Count()} 物件數量 : {usePart.Length}");
                    }
                    Debug.WriteLine($"---------------------------------------------------------------");
                    Debug.WriteLine($"使用權種分析訊息 : ");
                    Debug.WriteLine($"{wPopulation.ToString()}");
                    foreach (var item in wPopulation.GroupLength())
                    {
                        Debug.WriteLine($"長度 : {item.Key} 數量 : {item.Count()} 物件數量 : {weightsPatr.Length}");
                    }
                    Debug.WriteLine($"---------------------------------------------------------------");
                }
            }
