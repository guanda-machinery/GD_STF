        ...
        public unsafe void 完整自動配料(string projectName)
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                fixed (SinglePart* pUse = usePart)//使用率列表指標
                {
                    ...
                    Debug.WriteLine($"---------------------------------------------------------------");
                    using (StreamWriter stream = new StreamWriter($@"{ApplicationVM.DirectoryModel()}\比率{profiles[i].Replace('*', 'X')}.txt")) //將配料寫入檔案中
                    {
                        foreach (var item in wPopulation.GroupLength())
                        {
                            Debug.WriteLine($"長度 : {item.Key} 數量 : {item.Count()}");
                            ...
                            {
                                item.GetEnumerator().Reset();
                                while (item.GetEnumerator().MoveNext())
                                {
                                    stream.WriteLine($"{item.GetEnumerator().Current}");
                                    GD_STD.Data.Chromosome _ = item.GetEnumerator().Current;
                                    for (int q = 0; q < _.DNA.Count; q++)
                                    {
                                        stream.WriteLine($"{usePart[item.GetEnumerator().Current.DNA[q]]}");
                                    }
                                }
                                stream.WriteLine("----------------");
                            }
