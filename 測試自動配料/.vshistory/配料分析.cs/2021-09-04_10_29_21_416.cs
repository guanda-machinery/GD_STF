...
namespace 測試自動配料
{
    ...
    public class 配料分析
    {
        ...
        {
            SinglePart[] a = new SinglePart[1];
            fixed (SinglePart* usePart = a)
            {
                Chromosome chromosome = new Chromosome(100, new MatchSetting(), 10, usePart, 1);
                ...
                using (Stream objectStream = new MemoryStream())
                {
                    ...
                    //將自己所有資料序列化
                    formatter.Serialize(objectStream, chromosome);
                    //複寫資料流位置，返回最前端
                    objectStream.Seek(0, SeekOrigin.Begin);
                    //再將objectStream反序列化回去 
                    Chromosome _ = (Chromosome)formatter.Deserialize(objectStream);
                }
            }
        }
        ...
        public unsafe void 完整配料(string projectName)
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                fixed (SinglePart* pUse = usePart, pWeights = weightsPatr, pAverage = averagePart, pmUse = multipleusePart)//指標
                {
                    ...
                    wPopulation.Run();
                    GD_STD.Data.Population aPopulation = new GD_STD.Data.Population(matchSetting, pAverage, averagePart.Length, new Average());//權重配料
                    aPopulation.Run();
                    GD_STD.Data.Population umPopulation = new GD_STD.Data.Population(matchSetting, pmUse, multipleusePart.Length, new MultipleUsageRate());//分裂使用率配料
                    umPopulation.Run();
                    ...
                    Debug.WriteLine($"{uPopulation.ToString()} 配料物件 : {multipleusePart.Length}");
                    foreach (var item in umPopulation.GroupLength())
                    {
                        Debug.WriteLine($"長度 : {item.Key} 數量 : {item.Count()}");
                    }
                    Debug.WriteLine($"---------------------------------------------------------------");
        ...
        [Test, TestCaseSource("GetBom")]
        public unsafe void 分裂配法(string projectName)
        {
            MatchSetting matchSetting = new MatchSetting()//配料設定檔
            {
                MainLengths = new List<double>()
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                SinglePart[] multipleusePart = new SinglePart[weightsPatr.Length];//使用率的分裂零件列表
                Array.Copy(weightsPatr, multipleusePart, weightsPatr.Length);//複製到使用率
                fixed (SinglePart* pmUse = multipleusePart)//使用率列表指標
                {
                    GD_STD.Data.Population umPopulation = new GD_STD.Data.Population(matchSetting, pmUse, multipleusePart.Length, new MultipleUsageRate());//使用率配料
                    umPopulation.Run();
                    Debug.WriteLine($"{profiles[i]}");
                    Debug.WriteLine($"分裂使用率分析訊息 : ");
        ...
        [Test, TestCaseSource("GetBom")]
        public unsafe void 單一優化(string projectName)
        {
            MatchSetting matchSetting = new MatchSetting()//配料設定檔
            {
                MainLengths = new List<double>()
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                fixed (SinglePart* pAverage = averagePart)//權重列表指標
        ...
        public unsafe class Chromosome : IMatchSetting
        {
            ...
            #region 私有屬性
            //[NonSerialized]
            private SinglePart* Ptr { get; set; }
