    ...
    public class 配料分析
    {
        ...
        public unsafe void 完整自動配料(string projectName)
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                SinglePart[] countPart = new SinglePart[weightsPatr.Length];//數量評分的零件列表
                Array.Copy(weightsPatr, usePart, weightsPatr.Length);//複製到使用率
                Array.Copy(weightsPatr, countPart, weightsPatr.Length);//複製到數量評分
                fixed (SinglePart* pWeights = weightsPatr)//權重列表指標
                fixed (SinglePart* pAverage = countPart)//權重列表指標
                fixed (SinglePart* pUse = usePart)//使用率列表指標
                {
                    ...
                    wPopulation.Run();
                    GD_STD.Data.Population cPopulation = new GD_STD.Data.Population(matchSetting, pAverage, weightsPatr.Length, new Average());//權重配料
                    cPopulation.Run();
