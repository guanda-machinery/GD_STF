...
namespace 測試自動配料
{
    ...
    public class 配料分析
    {
        ...
        public unsafe void 自動配料(string projectName)
        {
            ...
            intPtr = Marshal.AllocHGlobal(Marshal.SizeOf(typeof(SinglePart)) * singleParts.Count);
            byte[] bytes = singleParts.ToArray().ToByteArray();
            ...
            {
                Population pop = new Population(matchSetting, (pSinglePart*)intPtr.ToPointer(), singleParts.Count);
                pop.Initialize();
        ...
        public unsafe class Population : List<Chromosome>
        {
            ...
            public Population(MatchSetting match, pSinglePart* ptr, int count)
            {
                _Match = match;
                _SingleParts = ptr;
                _Count = count;
            ...
            public void Initialize()
            {
                ...
                {
                    this.Add(new Chromosome(lengths[i], _Match, 0, _SingleParts, _Count));
                }
            }
            /// <summary>
            /// 取得最合適的物件
        ...
        public unsafe class Chromosome : IMatchSetting
        {
            ...
            public Chromosome(double length, IMatchSetting setting, int index, pSinglePart* parts, int count)
            {
