        ...
        public unsafe void 完整自動配料(string projectName)
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                fixed (SinglePart* pUse = usePart)//使用率列表指標
                {
                    ...
                    Debug.WriteLine($"---------------------------------------------------------------");
                    using (StreamWriter stream = new StreamWriter($@"{ApplicationVM.DirectoryModel()}\權重{profiles[i].Replace('*', 'X')}.txt")) //將配料寫入檔案中
                    {
                        for (int c = 0; c < wPopulation.Count; c++)
                        {
                            stream.WriteLine(wPopulation[i].ToCsvString(profiles[i]));
                        }
                        //foreach (var item in wPopulation.GroupLength())
                        //{
                    ...
                    using (StreamWriter stream = new StreamWriter($@"{ApplicationVM.DirectoryModel()}\使用率{profiles[i].Replace('*', 'X')}.txt")) //將配料寫入檔案中
                    {
                        for (int c = 0; c < uPopulation.Count; c++)
                        {
                            stream.WriteLine(uPopulation[i].ToCsvString(profiles[i]));
                        }
                        //foreach (var item in uPopulation.GroupLength())
                        //{
                        //    stream.WriteLine($"長度 : {item.Key} 數量 : {item.Count()}");
