    ...
    public class 配料分析
    {
        ...
        public unsafe void 分裂配法(string projectName)
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                fixed (SinglePart* pUse = usePart , pWeights = weightsPatr, pAverage = averagePart)//使用率列表指標
                {
                    GD_STD.Data.Population uPopulation = new GD_STD.Data.Population(matchSetting, pUse, usePart.Length, new MultipleUsageRate());//使用率配料
                    uPopulation.Run();
