...
namespace 測試自動配料
{
    ...
    public class 配料分析
    {
        ...
        /// <returns></returns>
        [DllImport("msvcrt.dll",SetLastError = true, EntryPoint = "LocalSize")]
        public static extern int _msize(IntPtr msize);
        [Test]
        public unsafe void 測試_msize()
        {
            SinglePart[] usePart = new SinglePart[10];//使用率的零件列表
            fixed (SinglePart* a = usePart)
            {
                IntPtr intPtr = (IntPtr)a;
                int aaa = _msize(intPtr) / sizeof(SinglePart);
            }
        }
        /// <summary>
        /// 單完測試結束
