    ...
    public class 配料分析
    {
        ...
        public unsafe void 完整自動配料(string projectName)
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                fixed (SinglePart* pCount = countPart)//權重列表指標
                fixed (SinglePart* pUse = usePart)//使用率列表指標
                {
                    ...
                    wPopulation.Run();
                    GD_STD.Data.Population cPopulation = new GD_STD.Data.Population(matchSetting, pCount, weightsPatr.Length, new Average());//權重配料
                    cPopulation.Run();
