...
namespace 測試自動配料
{
    ...
    public class 配料分析
    {
        ...
        public unsafe void 自動配料(string projectName)
        {
            ...
            int size = Marshal.SizeOf(parts[0]) * parts.Length;
            //int asize = Marshal.SizeOf(typeof(pSinglePart));
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                fixed (void* p = &parts[i])
                {
                    ...
                    Marshal.WriteIntPtr(intPtr, (IntPtr)p);
                }
            }
            SinglePart* AA = (SinglePart*)intPtr.ToPointer();
            var A = AA[parts.Length - 1].Length;
