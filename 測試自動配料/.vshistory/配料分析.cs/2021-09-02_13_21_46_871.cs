    ...
    public class 配料分析
    {
        ...
        public unsafe void 完整自動配料(string projectName)
        {
            ...
            for (int i = 0; i < profiles.Count; i++)
            {
                ...
                SinglePart[] weightsPatr = listPart.ToArray(); //複製陣列給指標
                for (int c = 0; c < weightsPatr.Length; c++)//寫入 index
                {
                    weightsPatr[i].ChangeIndex(i);
                }
        ...
        public unsafe class Population
        {
            ...
            /// 比較種子(種子集合)
            /// </summary>
            public List<Chromosome> Seed { get; set; }
            #endregion
            /// <summary>
            /// 標準建構式
            /// </summary>
            ...
            public void Initialize()
            {
                ...
                {
                    Seed.Add(new Chromosome(lengths[i], _Match, index, Ptr, _Count));//將主要配料長度加入到種子列表內
                }
            }
            /// <summary>
            /// 取得最合適的物件
        ...
        public unsafe class Chromosome : IMatchSetting
        {
            ...
            public List<SinglePart> Match()
            {
                ...
                for (int i = 0; i < _Count; i++) //逐步看動態指標
                {
                    if (Ptr[i].Length < max + Cut && !Ptr[i].Match) //找尋可配置的長度
                    {
                        if (!result.Contains(Ptr[i])) //如果列表內沒有這種長度
                        {
                            result.Add(Ptr[i]);//加入物件
                        }
                    }
                }
            ...
            public SinglePart? SingleMatch()
            {
                ...
                for (int i = 0; i < _Count; i++)//逐步看動態指標
                {
                    if (Ptr[i].Length < max + Cut && !Ptr[i].Match && !DNA.Contains(i)) //找尋可配置的長度
                    {
                        result = Ptr[i]; //指回傳找到第一個判斷是適合的物件
                        return result;
                    }
                }
                if (result == null)
            ...
            {
                double result = 0;
                for (int i = 0; i < DNA.Count; i++) //逐步查詢 index 
                {
                    result += Ptr[DNA[i]].Length; //加總物件長度
                }
            ...
            /// </summary>
            /// <returns>比數</returns>
            public double CoefficientFitness()
            {
                double fitness = Fitness(); //使用率
                if (fitness != 0)
                {
                    fitness /= DNA.Count; //比數
                }
                return fitness;
            }
            /// <summary>
