...
namespace 測試自動配料
{
    ...
    public class 配料分析
    {
        ...
        public unsafe void 自動配料(string projectName)
        {
            ...
            List<pSinglePart> singleParts = new List<pSinglePart>();//配料列表
            int index = 0;
            foreach (var item in steels)//將物件全部變成單一個體
            {
                singleParts.AddRange(pSinglePart.UnfoldPart(item, ref index));//展開物件並加入配料列表內
            }
            ...
            pSinglePart[] parts = singleParts.ToArray();
            singleParts.Sort(Compare);
            int size = sizeof(pSinglePart) * singleParts.Count;
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                ...
                {
                    int offset = sizeof(pSinglePart) * i;
                    Marshal.WriteIntPtr(intPtr, offset, (IntPtr)p);
                }
            }
            pSinglePart* AA = (pSinglePart*)intPtr.ToPointer();
            var A =  AA[0].Length;
            List<Chromosome> chromosomes = new List<Chromosome>();
            while (singleParts.Count != 0)
            {

                Population pop = new Population(matchSetting, (pSinglePart*)intPtr.ToPointer());
                pop.Initialize();
        ...
        /// <param name="b"></param>
        /// <returns></returns>
        private static int Compare(pSinglePart a, pSinglePart b)
        {
        ...
        public unsafe class Population : List<Chromosome>
        {
            ...
            [NonSerialized()]
            private pSinglePart* _SingleParts;
            private int _Size;
            #endregion
            /// <summary>
            /// 標準建構式
            ...
            public Population(MatchSetting match, pSinglePart* ptr)
            {
                _Match = match;
                _SingleParts = ptr;
            ...
            public void Initialize()
            {
                ...
                pSinglePart max = _SingleParts[0];
            ...
            public Chromosome Suitable()
            {
                while (!this.TrueForAll(el => el.End))
                {
                    for (int i = 0; i < this.Count; i++)
                    {
                        ...
                        {
                            List<pSinglePart> parts = this[i].Match();
                            if (parts != null)
                            {
        ...
        public unsafe class Chromosome : IMatchSetting
        {
            ...
            public Chromosome(double length, IMatchSetting setting, int index, pSinglePart* parts)
            {
            ...
            /// </summary>
            /// <returns></returns>
            public List<pSinglePart> Match()
            {
                double max = Length - UseLength(); //最大可配置長度
                List<pSinglePart> result = new List<pSinglePart>();
                for (int i = 0; i < _msize((IntPtr)ptr) / Marshal.SizeOf(typeof(pSinglePart)); i++)
                {
