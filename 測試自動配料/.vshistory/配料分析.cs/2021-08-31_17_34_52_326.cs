    ...
    public class 配料分析
    {
        ...
        public unsafe void 自動配料(string projectName)
        {
            ...
            fixed (SinglePart* ptr = arrayPatr)
            {
                ...
                {
                    Debug.WriteLine(chromosomes[i].ToString());
                    string strDNA = string.Empty;
                    for (int c = 0; c < chromosomes[i].DNA.Count; c++)
                    {
                        strDNA += ptr[chromosomes[i].DNA[c]].ToString() + "\n";
                    }
                    Console.WriteLine(strDNA);
                }
            }
