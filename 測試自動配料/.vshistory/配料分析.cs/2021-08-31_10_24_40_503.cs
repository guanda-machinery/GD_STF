...
namespace 測試自動配料
{
    ...
    public class 配料分析
    {
        ...
        public void 自動配料(string projectName)
        {
            ...
            byte[] result = singleParts.ToByteArray();
            IntPtr intPtr = Marshal.AllocHGlobal(result.Length);
            Marshal.StructureToPtr(result, intPtr, true);
            List<Chromosome> chromosomes = new List<Chromosome>();
            while (singleParts.Count != 0)
            {
                Population pop = new Population(matchSetting, intPtr);
                pop.Initialize();
        ...
        public unsafe class Population : List<Chromosome>
        {
            ...
            #endregion
            private SinglePart* _SingleParts { get; set; }
            ...
            public Population(MatchSetting match, IntPtr ptr)
            {
                _Match = match;
                SingleParts = (SinglePart*)ptr.ToPointer();
                IntPtr.Size;
            }
        ...
        public class Chromosome : IMatchSetting
        {
            ...
            public Chromosome(double length, IMatchSetting setting, int index)
            {
