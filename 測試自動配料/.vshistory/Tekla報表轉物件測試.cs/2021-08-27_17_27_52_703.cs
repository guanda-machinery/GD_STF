...
namespace 測試自動配料
{
    ...
    public class 分析
    {
        ...
        public void 報表()
        {
            ...
            //判斷資料數量與輸出數量有沒有相符
            int partFileCount= new DirectoryInfo(ApplicationVM.DirectorySteelPart()).GetFiles().Length;
            int boltFileCount = new DirectoryInfo(ApplicationVM.DirectorySteelBolts()).GetFiles().Length;
            Assert.AreEqual(part, partFileCount);
            Assert.AreEqual(bolt, boltFileCount);

            ObservableCollection<string> profileList = serialization.GetProfile();
            string dirPath = ApplicationVM.DirectorySteelPart();
            for (int i = 0; i < profileList.Count; i++)
            {
                serialization.GetPart(profileList[i]);
            }
