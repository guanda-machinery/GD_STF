...
namespace 測試自動配料
{
    ...
    public class 分析
    {
        ...
        public void 報表()
        {
            ...
            int dirPart = new DirectoryInfo(ApplicationVM.DirectorySteelPart()).GetFiles().Length;
            int dirBolt = new DirectoryInfo(ApplicationVM.DirectorySteelBolts()).GetFiles().Length;
            Assert.AreEqual(part, dirPart);
            Assert.AreEqual(bolt, dirBolt);
