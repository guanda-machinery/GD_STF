...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale zh-chs /uninstall /silent /vendor "Vendor Name" /productName "A Sandcastle Documented Class Library" /mediaBookList "A Sandcastle Documented Class Library" > NUL
...
REM build time.
IF EXIST "GuandaMachineryHelper.msha" COPY /Y "GuandaMachineryHelper.msha" HelpContentSetup.msha

REM Install the new content.
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale zh-chs /sourceMedia "%CD%\HelpContentSetup.msha"
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale zh-chs /wait 0 /operation uninstall /vendor "Vendor Name" /productName "A Sandcastle Documented Class Library" /bookList "A Sandcastle Documented Class Library" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale zh-chs /wait 0 /operation install /sourceUri "%CD%\GuandaMachineryHelper.msha"
