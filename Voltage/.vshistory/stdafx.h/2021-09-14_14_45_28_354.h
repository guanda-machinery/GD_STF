...
BOOL LocalSysCreateNamedShm(TCHAR szName[], HANDLE* phShmHandle, void** ppUserSpace, unsigned long ulSize)
{
	...
	*phShmHandle = OpenFileMappingA(FILE_MAP_WRITE, TRUE, (LPCSTR)szName);
	if (phShmHandle == NULL)
	{
		printf("錯誤");
	}
	if (*phShmHandle == 0)
	{
		*phShmHandle = CreateFileMappingA((HANDLE)0xffffffff, NULL, PAGE_READWRITE, 0, ulSize, (LPCSTR)szName);
	}
