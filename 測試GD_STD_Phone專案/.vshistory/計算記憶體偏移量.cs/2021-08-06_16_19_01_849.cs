...
namespace 測試GD_STD_Phone專案
{
    ...
    public class 計算記憶體偏移量
    {
        ...
        private static IEnumerable<TestMemoryOffset> TestOffsetDataSources()
        {
            ...
            {
                FieldName = $"{nameof(MonitorWork.WorkMaterial)}[1].{nameof(WorkMaterial.Material)}",
                Value = SharedMemory.GetMemoryOffset<WorkMaterial>(WorkMaterial.Initialization(), nameof(WorkMaterial.Material)) + Marshal.SizeOf(typeof(WorkMaterial))
            };
        ...
        public void 測試欄位參數是否符合條件(TestMemoryOffset testMemoryOffset)
        {
            ...
            long i = SharedMemory.GetMemoryOffset<MonitorWork>(monitorWork, testMemoryOffset.FieldName);

            Assert.AreEqual(i, testMemoryOffset.Value);
        }
        /// <summary>
        /// 每一個測試單元都會進入這方法
        /// </summary>
