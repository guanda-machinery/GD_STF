...
namespace 測試GD_STD_Phone專案
{
    ...
    public class 測試APP_Struct寫入記憶體結構
    {
        ...
        private static IEnumerable<TestObjectInfo> TestFieldDataSources()
        {
            ...
            #region 範圍
            //移動料架條件
            yield return new TestObjectInfo(nameof(APP_Struct.Count), 1, null, null, PhoneConditionAttribute.CONDITION.LIMITS);
            #endregion
        ...
        public void 測試欄位參數是否符合條件(TestObjectInfo testObjectInfo)
        {
            ...
            if (testObjectInfo.secondFieldName != null)
            {
                ...
                {
                    secondField.SetValue(readMemoryValue, true);
                }
            }
            ...
            try
            {
                factory.Field(testObjectInfo.mainFieldName);
            }
