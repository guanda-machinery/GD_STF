...
namespace 測試GD_STD_Phone專案
{
    ...
    public class 測試APP_Struct寫入記憶體結構
    {
        private static IEnumerable<TestObjectInfo> TestFieldDataSources()
        {
            ...
            yield return GetTestObjectInfo(nameof(APP_Struct.AxisSelect), true, nameof(APP_Struct.Origin));
            ...
            yield return GetTestObjectInfo(nameof(APP_Struct.OpenOil), false, nameof(APP_Struct.OpenOil));
            ...
            yield return GetTestObjectInfo(nameof(APP_Struct.HighSpeed), true, nameof(APP_Struct.Origin));
            ...
            yield return GetTestObjectInfo(nameof(APP_Struct.RollMove), MOBILE_RACK.INSIDE, nameof(APP_Struct.OpenOil));
            yield return GetTestObjectInfo(nameof(APP_Struct.RollDeepDrop), MOBILE_RACK.INSIDE, nameof(APP_Struct.OpenOil));

            //移動料架條件
            yield return GetTestObjectInfo(nameof(APP_Struct.HighSpeed), true, nameof(APP_Struct.SelectMove));
        ...
        {
            bool value = Convert.ToBoolean(typeof(APP_Struct).GetField(conditionFieldName).
                                                                   GetCustomAttributes<PhoneConditionAttribute>().
                                                                       FirstOrDefault(el => el.FieldName == conditionFieldName).Value);
        ...
        public void 單一測試欄位(TestObjectInfo testObjectInfo)
        {
            ...
            APP_Struct writeMemortyValue = APP_Struct.Initialization();
            APP_Struct readMemoryValue = APP_Struct.Initialization();

            typeof(APP_Struct).GetField(testObjectInfo.writeFieldName).SetValue(writeMemortyValue, testObjectInfo.writefieldValue);
            typeof(APP_Struct).GetField(testObjectInfo.writeFieldName).SetValue(readMemoryValue, testObjectInfo.conditionFieldValue);


            Console.WriteLine();

            PhoneConditionFactory<APP_Struct> factory = new PhoneConditionFactory<APP_Struct>(writeMemortyValue, readMemoryValue);
            Assert.Throws<MemoryException>(() => factory.Field())
            Assert.Pass();
            Dispose();
            //APP_Struct
