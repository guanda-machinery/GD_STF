...
namespace 測試GD_STD_Phone專案
{
    public class TestObjectInfo
    {
        ...
        public TestObjectInfo(string mainFieldName, object mainValue, string secondValueName, object secondValue)
        {
    ...
    public class 測試APP_Struct寫入記憶體結構
    {
        ...
        private static IEnumerable<TestObjectInfo> TestFieldDataSources()
        {
            ...
            #region 範圍
            //移動料架條件
            yield return new TestObjectInfo(nameof(APP_Struct.Count), 1, null, null);
            #endregion
        ...
        public void 測試欄位參數是否符合條件(TestObjectInfo testObjectInfo)
        {
            ...
            FieldInfo fieldInfo = typeof(APP_Struct).GetField(testObjectInfo.secondFieldName);
            fieldInfo.SetValue(readMemoryValue, testObjectInfo.secondFieldValue);
            ...
            {
                Console.Write($"欄位 {testObjectInfo.mainFieldName} 測試失敗");
            }
            Dispose();
            //APP_Struct
        }
