    ...
    public class PhoneConditionFactory<T> where T : struct, IPhoneSharedMemory
    {
        ...
        public void Field(string fieldName)
        {
            ...
            field.GetCustomAttributes<PhoneConditionAttribute>()?.ToList()
                  .ForEach(el =>
                  {
                      string _CompareValue = GetValue(field.GetValue(_Compare));//準備寫入記憶體的值
                      string _InitialValue = GetValue(field.GetValue(_Initial));//記憶體目前的值
                      if (!el.WriteField)
                      {
        ...
        /// <param name="attribute"></param>
        /// <param name="field"></param>
        /// <param name="compareValue">準備寫入記憶體的值</param>
        /// <param name="initialValue">記憶體目前的值</param>
