...
namespace GD_STD.Base
{
    ...
    ///             <item><see cref="SetValue{T, TVaule}(long, TVaule, long, long)"/></item>
    ///         </list>
    ///     </revision>
    /// </revisionHistory>
    public static partial class SharedMemory
    {
        ...
        /// </summary>
        /// <typeparam name="T">寫入記憶體結構型態</typeparam>
        /// <typeparam name="TVaule">寫入參數型態</typeparam>
        /// <param name="position">會在此處開始寫入存取子的位元組數</param>
        /// <param name="value">寫入的值</param>
        /// <param name="valueOffset"></param>
        /// <param name="size"></param>
        /// <returns></returns>
        public static T SetValue<T, TVaule>(long position, TVaule value, long valueOffset, long size) where T : IPhoneSharedMemoryOffset, ISharedMemory, new() where TVaule : struct
        {
            T result = new T();
            result.WriteMemory(position, value, valueOffset, size);
            result.ReadMemory();
            return result;
        }
        /// <summary>
