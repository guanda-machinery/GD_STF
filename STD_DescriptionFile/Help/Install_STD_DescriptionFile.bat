...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale zh-chs /uninstall /silent /vendor "Vendor Name" /productName "STDMemoryLibrary" /mediaBookList "STDMemoryLibrary" > NUL
...
REM build time.
IF EXIST "STD_DescriptionFile.msha" COPY /Y "STD_DescriptionFile.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale zh-chs /wait 0 /operation uninstall /vendor "Vendor Name" /productName "STDMemoryLibrary" /bookList "STDMemoryLibrary" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale zh-chs /wait 0 /operation install /sourceUri "%CD%\STD_DescriptionFile.msha"
