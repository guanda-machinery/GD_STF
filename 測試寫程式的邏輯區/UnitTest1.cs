...
namespace 測試寫程式的邏輯區
{
    public class Tests
    {
        ...
        private static IEnumerable<List<int>> 測試連續數字資料()
        {
            yield return new List<int> { 2, 5, 4, 7 };
            yield return new List<int> { 0, 6, 7, 8, 9 };
            yield return new List<int> { 0 };
            yield return new List<int> { 1 };
            yield return new List<int> { 1, 3 };
            yield return new List<int>();
        }
        [Test, TestCaseSource("測試連續數字資料")]
        public void 連續數字(List<int> vs)
        {
